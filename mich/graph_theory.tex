        ...
        \item Can think in terms of the following picture (WIP). Eventually, we will run out of colours and get a monochromatic triangle.
            \begin{center}
                \begin{tikzpicture}[scale=0.6]
                    ...
                    \node[node, label={$w$}] (w) at (-4, 1) {};

                    \foreach \theta in {24, 22, ..., -24}{
                        \pgfmathsetmacro\radius{3.5+rand*0.5}
                        \pgfmathsetmacro\angle{\theta-rand*0.6}

                        \ifthenelse{\theta = 14}
                            { \draw[bred] (v) -- (w) }
                            { \draw[bred] (v) -- ++(\angle: \radius) };
                        \draw[bgreen] (w) -- ++(\angle: \radius);
                    }
                    ...
                    \item $v$ is in \green{$6$ green} edges
                        \begin{center}
                            \begin{tikzpicture}[scale=0.4]
                                % TODO think about the scale here after the big diagram above is done
                                \node [node, label=left:$v$] (v) at (0, 0) {};
                        ...
                        We know $H$ contains a monochromatic triangle. If it is \red{red}, we are done.  If it is \green{green}, then it makes a \green{green $K_4$} with $v$ as required.
                \end{itemize}
            \end{proof}
...
\begin{remark}
    \leavevmode
    \begin{itemize}
        \item A more careful version of the same proof gives $R(s) = \mathcal{O} (\frac{4^s}{\sqrt{s}})$ (Erdos-Szekeres 1935)
        \item A harder proof gives an improvement to $R(s) = \mathcal{O}(\frac{4^s}{s})$ (Thomason 1988)
        \item An even harder proof gives $R(s) = \mathcal{O}(\frac{4^s}{s^k})$ for any $k$ (Conlon 2009)
        \item Is $R(s) = \mathcal{O}((3.999999999)^s)$? Unknown.
    \end{itemize}
\end{remark}
...
\begin{proof}
    ...
    Let $n = R(s, m)$ and colour $K_n$ with $k$ colours, including \red{red} but not green. Re-colour by turning all non-\red{red} edges \green{green}. By definition, of $n$, we have either
    \begin{itemize}
...
\begin{proof}
    ...
    If $n$ is sufficiently large (as we may assume) then $m \geq n t$ and so $\frac{m}{n} \geq t$ and so $\frac{2m}{n} - t \geq \frac{m}{n}$.  Thus $n^t (t-1) \geq n (\frac{m}{n})^t$ so $m^t \leq n^{2t-1} (t-1)$ so $m \leq (t-1)^\frac{1}{t} n^{2-\frac{1}{t}} = \mathcal{O}(n^{2-\frac{1}{t}})$.
\end{proof}
...
What is $\ext(n; H)$? This is a hard question, and we could not even find an exact answer for $C_4$, so instead consider some asymptotics.
...
\begin{proof}
    Let $n > \abs{H}$, let $\abs{G} = n$, $e(G) = \ext(n; H) = x_n \binom{n}{2}$ and $H \not\subset G$. For any $v \in G$, $\abs{G - v} = n-1$ and $H \not\subset G - v$, so $e(G-v) \leq \ext(n-1; H) \ x_{n-1} \binom{n-1}{2}$.
    Each edge $xy \in E(G)$ in $G-v$ for all $v \neq x, y$. Hence $(n-2) x_n \binom{n}{2} = (n-2) e(G) = \sum_{v \in G} e(G-v) \leq n x_{n-1} \binom{n-1}{2}$. So $x_n \leq x_{n-1}$, so $(x_n)$ is decreasing and bounded below by zero.
\end{proof}
