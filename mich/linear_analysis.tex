...
\begin{thm}[Stone-Weierstrass Approximation Theorem]
...
\end{prop}
\begin{proof}
    By the complex Stone-Weierstrass Theorem, for any $\epsilon > 0$, there is a trig poly $P \in A$ such that $\norm{f-P}_\infty < \epsilon$.
...
\begin{lemma}
    ...
    \end{equation*}
    is bounded and compact from $(C[a,b],\norm{\cdot}_2)$ to $(C[a,b],\norm{\cdot}_\infty)$.
\end{lemma}

\begin{proof}
    By Cauchy-Schwarz,
    \begin{equation*}
        \norm{Kf}_\infty \leq \sup_x \int \abs{k(x,y)} \abs{f(y)} \, dy \leq \underbrace{\sup_x (\int \abs{k(x,y)}^2 \, dy)^\frac{1}{2}}_{\leq C} \norm{f}_2
    \end{equation*}
    so $K$ is bounded from $(C[a,b],\norm{\cdot}_2)$ to $(C[a,b],\norm{\cdot}_\infty)$.
    Also $\set{K f | f \in C[a,b], \norm{f}_2 \leq 1}$ is equicontinuous:
    \begin{equation*}
        \abs{K f(x) - K f(y)} \leq (\underbrace{\int_a^b \abs{k(x,z) - k(y,z)}^2 \, dz}_{\to 0 \text{ as } x \to y})^\frac{1}{2} \norm{f}_2 \to 0
    \end{equation*}
    uniformly in $f$.
...
\begin{cor}
    $K$ is bounded and compact from $(C[a,b], \norm{\cdot}_2)$ to $(C[a,b], \norm{\cdot}_2)$.
\end{cor}

\begin{proof}
    This follows from
    \begin{equation*}
        \norm{Kf}_2 \leq (\int_a^b \abs{K f(x)}^2 \, dx)^\frac{1}{2} \leq \sqrt{\abs{a-b}} \norm{K f}_\infty. \qedhere
    \end{equation*}
\end{proof}
...
\begin{thm}
    ...
    \begin{enumerate}[label=\alph*]
        \item $\norm{\Phi(f)} \leq \norm{f}$ (in particular $\Phi$ is continuous)
...
\begin{lemma}
    Let $T \in \mathcal{B}(H)$ be self-adjoint. Then
    \begin{equation*}
        \norm{T} = \sup_{\lambda \in \sigma(T)} \abs{\lambda}. \label{eq:24dagger} \tag{$\dagger$}
    \end{equation*}
...
\begin{proof}
    ...
    Thus $\norm{T} \leq \sup_{\lambda \in \sigma(T)} \abs{\lambda}$.
    We also saw that $\sigma(T) \subset \set{z \in \C | \abs{z} \leq \norm{T}}$.
    Thus $\norm{T} \geq \sup_{\lambda \in \sigma(T)} \abs{\lambda}$.

    Now:
    \begin{align*}
        \norm{P(T)}^2 = \sup_{\norm{x} \leq 1} \norm{P(T) x}^2 &= \sup_{\norm{x} \leq 1} (x, P(T)^* P(T) x) \\
                                                               &= \norm{P(T)^* P(T)} \\
                                                               &= \norm{(\overline{P}P) (T)} \\
...
\begin{proof}[Proof of theorem]
    ...
    Thus for $\lambda \in \sigma(T)$ there are $(x_n)$ with $\norm{x_n} = 1$ such that $f_n(T) x_n - f_n(\lambda) x_n \to 0$.
    \begin{align*}
...
\begin{cor}
    ...
    \begin{equation*}
        \norm{R_T(z)} = \frac{1}{\mathrm{dist}(z, \sigma(T))}.
    \end{equation*}
\end{cor}
\end{document}
