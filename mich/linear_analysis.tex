...
\begin{thm}[Arzel\'a-Ascoli]\hypertarget{thm:aa}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item $\mathcal{F}$ is bounded and equicontinuous, i.e. $\sup_{f \in \mathcal{F}} \|f\|_\infty < \infty$ and $\forall \epsilon > 0 \; \forall x \in K$, $\exists U$ a neighbourhood of $x$ such that
            \begin{equation*}
...
\begin{proof}
    ...
    Then if $f = f_{x_1} \vee f_{x_2} \vee \dotsb \vee f_{x_m}$, we have $f(z) > g(z) - \epsilon \quad \forall z$, and our first bound $f(z) < g(z) + \epsilon$ still holds, so $\|f - g\| < \epsilon$. % check the norm here
\end{proof}
