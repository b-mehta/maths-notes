...
\begin{defi}[\hypertarget{def:dualSpace}{Dual space}]
    Let $V$ be a \hyperlink{def:topologicalVectorSpace}{topological vector space}. The (topological) \textbf{dual space} of $V$ is the space of continuous linear maps $V \to \K$. Denote it by $V^*$. In a \hyperlink{def:normedVectorSpace}{normed space}, $V^* = \mathcal{B}(V, \K)$. (We call $\mathcal{L}(V, \K)$ the algebraic dual of $V$.)
\end{defi}
...
\begin{proof}
    Let $(T_i) \subset \mathcal(V, W)$ be a Cauchy sequence.  Then for any vector $v \in V$, the sequence $(T_i v) \subset W$ is Cauchy:
    \begin{equation}
        \norm{T_i v - T_j v} \leq \underbrace{\norm{T_i - T_j}}_{\leq \epsilon} \cdot \norm{v}
    \end{equation}
    Since $W$ is complete, there is $Tv \in W$ such that $T_i v \to T v$.
    \begin{itemize}
        \item $T$ is linear:
            \begin{align*}
...
\begin{fact}
    $T^* f$ is indeed in $V^* = \mathcal{B}(V, K)$ and $\norm{T^*} \leq \norm{T}$
\end{fact}
...
\begin{defi}
    Let $V$ be a normed vector space. The \textbf{double dual} of $V$ is the dual space of $V^*$, that is $V^{**} = (V^*)^*$.
\end{defi}
...
\begin{eg}
    \leavevmode
    \begin{itemize}
        \item Let $V, W$ be finite-dimensional vector spaces. Then any linear map $T:V \to W$ is bounded with respect to any norms on $V, W$, $T$ can be represented by a matrix and $T^*$ is represented by the transpose of $T$. Finite dimensional \hyperlink{def:normedVectorSpace}{normed vector spaces} are \hyperlink{def:reflexive}{reflexive}.
