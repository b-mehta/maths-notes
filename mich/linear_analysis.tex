...
\begin{defi}[\hypertarget{def:locallyConvex}{Locally convex}]
...
\begin{lemma}
    Let $V$ be a \hyperlink{def:topologicalVectorSpace}{topological vector space} and $C \subset V$ be a \hyperlink{def:bounded}{bounded}, \hyperlink{def:convexSubs}{convex} neighbourhood of $0$.
    Then there exists a bounded, \hyperlink{def:balanced}{balanced}, {convex} neighbourhood $\widetilde C$ of $0$.
\end{lemma}
...
\begin{prop}
    Let $V$ be a \hyperlink{def:topologicalVectorSpace}{topological vector space} and $C \subset V$ be a \hyperlink{def:bounded}{bounded}, \hyperlink{def:convexSubs}{convex} neighbourhood of $0$. Then the topology on $V$ is induced by a norm.
\end{prop}
...
\begin{cor}
    A \hyperlink{def:topologicalVectorSpace}{topological vector space} is normable iff it is \hyperlink{def:locallyConvex}{locally convex} and locally bounded (ie there exists a bounded convex neighbourhood of $0$).
\end{cor}

\begin{defi}\hypertarget{def:banach}
...
\begin{eg} \leavevmode
    \begin{enumerate}[(i)]
        \item Any finite dimensional vector space is a Banach space (with any norm).
        \item Let $X$ be a set, and $\mathcal{B}(X)$ the set of bounded $\K$-valued functions on $X$.  Then $\mathcal{B}(X)$ is a Banach space with norm
            \begin{equation*}
        ...
        \item Let $U \subset \R^n$ be open (not necessarily bounded) and denote by $\mathcal{C}(U)$ the space of continuous functions on $U$ (not necessarily bounded). Then $\mathcal{C}(U)$ is a topological vector space with topology generated as follows.
            ...
            Then the topology generated by the $V(i, n)$ and their translates makes $\mathcal{C}(U)$ a locally convex topological vector space, and the topology on $\mathcal{C}(U)$ is generated by the metric
            \begin{equation*}
...
\begin{fact}
    In any \hyperlink{def:topologicalVectorSpace}{topological vector spaces} $V$, $W$, a \emph{linear} map $T: V \to W$ is continuous if and only if it is continuous at $0$.
\end{fact}
\begin{proof}
    Let $T$ be continuous at $0$ and $v \in V$. Let $w = Tv$ and $U \subset W$ and open neighbourhood of $w$. Then $U - w$ is an open neighbourhood of $0 \in W$. Since $T$ is continuous at $0$, $T^{-1}(U - w)$ contains an open neighbourhood $U' \subset V$ of $0$.
    By linearity, $T(v + U') = Tv + T(U') \subset Tv + U - w = U$.  Since $v + U'$ is an open neighbourhood of $v$ this means that $T$ is continuous at $v$.
\end{proof}

\begin{defi}\hypertarget{def:boundedLinearMap}
...
\begin{fact}
    If $V, W$ are \hyperlink{def:normedVectorSpace}{normed vector spaces}, a linear map $T: V \to W$ is bounded iff there is $\lambda > 0$ such that
    \begin{equation*}
