...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
Note $\mathcal{P}(\omega) \in \hyperlink{def:vk}{V_{\omega+2}}$, $\mathcal{P}(\omega) \subseteq V_{\omega+1}$.
...
\begin{proof}[Proof of G\"odel's Theorem]\leavevmode
  \begin{description}
    ...
    \item[Pair] $x,y \in \hyperlink{def:l}{L_\kappa}$, find $\alpha$ such that $x,y \in L_\alpha$. $\{x,y\} \subseteq L_\alpha$.
      Clearly the formula $\varphi(z,x,y) \coloneqq z=x \land z=y$ defines the pair $\{x,y\}$, so by our Lemma earlier, the pair lies in $\hyperlink{def:d}{\mathcal{D}(L_\alpha)} = L_{\alpha+1} \subseteq L_\kappa$.
    ...
    Set $\alpha_0 \coloneqq \text{Ord} \cap L_\beta$, and $\alpha_{n+1}$ as the least $\gamma$ such that $L_\gamma$ contains $L_{\alpha_n}$ and a witness for each existential statement true in $L_\kappa$ with parameters in $L_{\alpha_n}$.
    Then
    \begin{align*}
      \bar{\alpha} &\coloneqq \bigcup_{n \in \mathbb{N}} \alpha_{n} \\
      L_{\overline{\alpha}} &\preccurlyeq L_{\kappa} \quad \text{with } \bar{\alpha} < \kappa.
    \end{align*}
    Define the set via $\varphi$ over $L_{\bar{\alpha}}$:
    \begin{align*}
      &\mathrel{\phantom{=}}\{z \in x \mid L_{\bar{\alpha}} \models \varphi(z,a_1,\dotsc,a_n)\} \\
...
\begin{prop}
  ...
  Then $p \forces \varphi \iff p \forces^* \varphi$.
\end{prop}
\begin{proof}
...
\begin{proof}[Proof of \hyperlink{thm:forcing}{Forcing Theorem}]
  ...
  \begin{enumerate}\setcounter{enumi}{2}
    ...
    \item[1.] $(\Leftarrow)$ Let $p \in G$ such that $p \forces^* \tau_1 \in \tau_2$.
      ...
      \begin{align*}
        \{q \leq p \mid \exists(\bar{\pi}, \bar{s}) \in \tau_2 \ (q \leq \bar{s} \land q \forces^* \bar{\pi} = \tau_1)\}
      \end{align*}
      This set, or even the smaller set
      \begin{align*}
        \{q \leq p \mid q \leq s \land q \forces^* \pi = \tau_1\}
      \end{align*}
