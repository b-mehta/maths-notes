  ...
  \begin{enumerate}[label=(\alph*)]\label{eg:4.2}
    ...
    \item\marginnote{\emph{Lecture 10}}[0cm]
      ...
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
      \index{span}\hypertarget{def:span}A \hyperlink{def:diagram}{diagram of shape $J$} is a pair of objects $A,B$; a \hyperlink{def:cone}{cone} over it is a \textbf{span}
      \begin{equation*}
      ...
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
    \item More generally, if $J$ is a \hyperlink{def:small}{small} discrete category, a diagram of shape $J$ is a $J$-indexed family $\set{A_j | j \in J}$, and a limit for it is a product
      \begin{equation*}
...
\begin{nthm}\label{thm:4.3}\leavevmode
  \begin{enumerate}[label=(\roman*)]
    \item Suppose $\mathscr{C}$ has \hyperlink{def:equalizer}{equalizers} and all finite (resp.\ \hyperlink{def:small}{small}) \hyperlink{def:prod}{products}.
      Then $\mathscr{C}$ has all finite (resp.\ small) \hyperlink{def:limit}{limits}.
    \item Suppose $\mathscr{C}$ has \hyperlink{def:pullback}{pullbacks} and a \hyperlink{def:terminal}{terminal object}.
      Then $\mathscr{C}$ has all finite limits.
  \end{enumerate}
\end{nthm}
\begin{proof}\leavevmode
  \begin{enumerate}[label=(\roman*)]
...
\begin{ndef}\label{def:4.4}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \hypertarget{def:rlim}We say $F$ \textbf{reflects limits}\index{limit!reflected}\index{reflects limits} of shape $J$ if, given $D: J \to \mathscr{C}$ and a cone $(L, (\lambda_j)_j)$ such that $(FL, \set{F\lambda_j | j \in \ob J})$ is a limit for $FD$, then $(L, (\lambda_j)_j)$ is a limit for $D$.
    \item \hypertarget{def:clim}We say $F$ \textbf{creates limits}\index{limit!created}\index{creates limits} of shape $J$ if, given $D: J \to \mathscr{C}$ and a limit $(M, \set{\mu_j | j \in \ob J})$ for $FD$, there exists a cone $(L, (\lambda_j)_j)$ over $D$ whose image under $F$ is isomorphic to the limit cone, and any such that cone is a limit in $\mathscr{C}$.
  \end{enumerate}
\end{ndef}
\printindex
\end{document}
