...
Given a \hyperlink{def:lop}{local operator} $j$ on $\ec$, we write $\sh_j(\ec)$ for the \hyperlink{def:fulls}{full subcategory} of $j$-\hyperlink{def:sheaf}{sheaves} in $\mathscr{E}$.

\begin{nlemma}\label{lem:7.19}
  $\sh_j(\ec)$ is closed under \hyperlink{def:limit}{limits} in $\ec$, and is an \hyperlink{def:expIdeal}{exponential ideal}.
\end{nlemma}
\begin{proof}
...
\begin{nlemma}\label{lem:7.20}
  If $A$ is a \hyperlink{def:sheaf}{sheaf}, then a \hyperlink{def:subobj}{subobject} $\begin{tikzcdi}A'\rar[tail]{m} & A\end{tikzcdi}$ in $\ec$ is a sheaf iff it is \hyperlink{def:denseclosed}{closed}.
\end{nlemma}
\begin{proof}\leavevmode
  \begin{itemize}
