...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]\label{1.7}
    ...
    \item Let $F: \mathbf{Set} \to \mathbf{Gp}$ be the \hyperlink{def:freeFunct}{free group functor} (\cref{1.5}(b)) and $U: \mathbf{Gp} \to \mathbf{Set}$ the forgetful functor.
...
\end{ndef}
\begin{eg}
  $\mathbf{Mat}_K$ is \hyperlink{def:skel}{skeletal}, and the image of $F: \mathbf{Mat}_K \to \mathbf{FdMod}_K$ of \cref{1.10}(c) is a \hyperlink{def:skel}{skeleton} of $\mathbf{FdMod}_K$.
\end{eg}
\begin{warning}
...
\begin{nexample}\label{eg:2.6}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    ...
    \item The functor $\mathcal{P}^*: \mathbf{Set}^{op} \to \mathbf{Set}$ (see \cref{1.5}(c)) is representable by $(\{0,1\}, \{1\})$: this is the bijection between subsets and characteristic functions.
...
\begin{nexample}\label{eg:2.10}\leavevmode
  ...
  \end{enumerate}
\end{nexample}
Note that any \hyperlink{def:funct}{functor} of the form $\hyperlink{def:lsmall}{\mathscr{C}(A, -)}$ preserves \hyperlink{def:monic}{monomorphisms}, but they don't normally preserve \hyperlink{def:epic}{epimorphisms}. % TODO: check this is true
...
\begin{proof}
  ...
  \end{equation*}
  The latter mapping is natural in $B$ since $G$ is a functor, and in $A$ since, by construction, $\eta$ is a natural transformation $1_\cc \to GF$.
\end{proof}
\begin{ncor}\label{cor:3.4}
  If $F$ and $F'$ are both \hyperlink{def:adj}{left adjoint} to $G: \dc \to \cc$, then they are \hyperlink{def:natiso}{naturally isomorphic}.
\end{ncor}
\begin{proof}
...
\begin{nlemma}\label{lem:3.5}
  ...
  \begin{equation*}
    \begin{tikzcd}
      \cc \rar[shift left]{F} & \dc \lar[shift left]{G} \rar[shift left]{H} & \lar[shift left]{K} \mathscr{E}
    \end{tikzcd}
  \end{equation*}
...
\end{ncor}
\begin{proof}
  By \cref{lem:3.5}, both ways round the diagram of left adjoints are left adjoint to the composite $\cc \to \mathscr{F}$, so by \cref{cor:3.4} they are isomorphic.
\end{proof}
\index{unit}\hypertarget{def:unit}Given an \hyperlink{def:adj}{adjunction} $(F \dashv G)$, the \hyperlink{def:nattrans}{natural transformation} $\eta: 1_{\cc} \to GF$ emerging in the proof of \cref{thm:3.3} is called the \textbf{unit} of the adjunction.
\index{counit}\hypertarget{def:counit}Dually, we have a natural transformation $\epsilon: FG \to 1_{\dc}$ such that $\epsilon_B: FGB \to B$ corresponds to $GB \overset{1_{GB}}\to GB$ is called the \textbf{counit}.
\begin{nthm}\label{thm:3.7}
  Given functors
  $\begin{tikzcd}
          \cc \rar[shift left]{F} & \dc \lar[shift left]{G}
  \end{tikzcd}$
  specifying an \hyperlink{def:adj}{adjunction} $(F \dashv G)$ is equivalent to specifying natural transformations $\eta: 1_{\cc} \to GF$, $\epsilon: FG \to 1_{\dc}$ satisfying the commutative diagrams
  \begin{equation*}
    \begin{tikzcd}
...
\begin{nlemma}\label{lem:3.8}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  and \hyperlink{def:natiso}{natural isomorphisms} $\alpha: 1_{\cc} \to GF$, $\beta: FG \to 1_{\dc}$.
  Then there are isomorphisms $\alpha': 1_{\cc} \to GF$, $\beta': FG \to 1_{\dc}$ which satisfy the \hyperlink{def:triId}{triangular identities}, so $\hyperlink{def:adj}{(F \dashv G)}$ (and $(G \dashv F)$).
\end{nlemma}
\begin{proof}
  We define $\alpha' = \alpha$ and $\beta'$ to be the composite
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      FG \rar{(FG\beta)^{-1}} & FGFG \rar{(F\alpha G)^{-1}} & FG \rar{\beta} & 1_{\dc}.
    \end{tikzcd}
  \end{equation*}
...
\end{proof}
\begin{nlemma}
  Suppose $G: \dc \to \cc$ has a \hyperlink{def:adj}{left adjoint} $F$ with \hyperlink{def:counit}{counit} $\epsilon: FG \to 1_{\dc}$, then
  \begin{enumerate}[label=(\roman*)]
