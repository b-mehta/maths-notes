...
\begin{equation*}\begin{tikzcd}[sep=small]A\dar \\ B\end{tikzcd}\end{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  is a pullback in $\mathscr{C}$, i.e.\ a product in $\mathscr{C}/B$.
...
\begin{ndef}\label{def:6.8}\hypertarget{def:ccfunct}
  ...
  where $\text{ev}$ is the counit of $((-) \times A \dashv (-)^A)$.
  We say $F$ is a \index{cartesian closed!functor}\textbf{cartesian closed functor} if $\theta$ is an isomorphism for every pair $(A,B)$.
\end{ndef}
...
\begin{nthm}\label{thm:6.9}
  ...
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      L(A \times FB) \rar{(L\pi_1, L\pi_2)} & LA \times LFB \rar{1_{LA} \times \epsilon_B} & LA \times B
    \end{tikzcd}
  \end{equation*}
