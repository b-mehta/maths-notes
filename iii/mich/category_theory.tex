...
\begin{nexample}\label{eg:1.3}\leavevmode
  \begin{enumerate}[label=(\alph*)]\hypertarget{def:categ}
    ...
    \item \index{duality}\hypertarget{def:duality}Given $\mathscr{C}$, the \textbf{opposite category} $\mathscr{C}^\text{op}$ has the same objects and morphisms as $\mathscr{C}$, but $\dom$ and $\cod$ are interchanged, and $f g $ in $\mathscr{C}^\text{op}$ is $gf$ in $\mathscr{C}$.
...
\hypertarget{def:haslimits}(We say $\mathscr{C}$ \textbf{has limits} of shape $J$).
...
\begin{nremark}\label{rem:4.5}\leavevmode\marginnote{\emph{Lecture 11}}[0cm]
  \begin{enumerate}[label=(\alph*)]
    \item If $\mathscr{C}$ has \hyperlink{def:limit}{limits of shape $J$}, $F: \mathscr{C} \to \mathscr{D}$ \hyperlink{def:plim}{preserves} them and $F$ and $F$ reflects \hyperlink{def:iso}{isomorphisms}, then $F$ \hyperlink{def:rlim}{reflects} limits of shape $J$.
...
\hypertarget{thm:paft}The `primeval' Adjoint Functor Theorem says that the converse of \cref{thm:4.9} is true: if $\mathscr{D}$ has and $G: \mathscr{D} \to \mathscr{C}$ \hyperlink{def:plim}{preserves} \emph{all} limits, then $G$ has a \hyperlink{def:adj}{left adjoint}.
...
\end{nlemma}
\begin{proof}
  First suppose $\cc$ has an \hyperlink{def:initial}{initial object} $I$.
  Then the unique morphisms $\set{I \to A | A \in \ob \cc}$ form a \hyperlink{def:cone}{cone} over $1_\cc$; and given any cone $\set{C \overset{\lambda_A}\to A | A \in \ob \cc}$, then for any $A$ the triangle
  \begin{equation*}
    \begin{tikzcd}
      C \rar{\lambda_I} \drar[']{\lambda_A} & I \dar \\& A
    \end{tikzcd}
  \end{equation*}
  commutes, so $\lambda_I$ is the unique factorization of $\set{\lambda_A | A \in \ob \cc}$ through $\set{I \to A | A \in \ob \cc}$.

  Conversely, suppose $(I, \set{\lambda_A: I \to A| A \in \ob \cc})$ is a limit.
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  commutes. In particular, putting $f = \lambda_A$, we see that $\lambda_I$ is a factorization of the limit cone through itself, so $\lambda_I = 1_I$.
  Hence every $f: I \to A$ satisfies $f = \lambda_A$.
\end{proof}

The \hyperlink{thm:paft}{primeval Adjoint Functor Theorem} follows immediately from \cref{lem:4.10}, \cref{lem:4.11} and \cref{thm:3.3}.
However, it only applies to functors between preorders (cf question 6 on example sheet 2).

\begin{nthm}[General Adjoint Functor Theorem]\label{thm:4.12gaft}
  Suppose that $\dc$ is \hyperlink{def:lsmall}{locally small} and \hyperlink{def:complete}{complete}.
  \hypertarget{def:ssc}Then $G: \dc \to \cc$ has a \hyperlink{def:adj}{left adjoint} if and only if $G$ satisfies the \named{solution set condition}: the solution set condition says that $G$ preserves all small limits and, for each $A \in \ob \cc$ there exists a set of morphisms $\set{A \overset{f_i}\to GB_i | i \in I}$ such that every $A \overset{h}\to GC$ factors as $A \overset{f_i}\to GB_i \overset{Gg}\to GC$ for some $i$ and some $g: B_i \to C$.
\end{nthm}
\begin{proof}
  $(\Rightarrow)$. If $F \dashv G$, $G$ preserves limits by \cref{thm:4.9}, and $\{A \overset{\eta_A}\to GFA\}$ is a singleton solution set, by \cref{thm:3.3}.

  $(\Leftarrow)$. By \cref{lem:4.10}, $(A \downarrow G)$ is complete, and it inherits local smallness from $\dc$.
  So we need to show: if $\mathscr{A} \coloneqq (A \downarrow G)$ is complete and locally small, and has a weakly initial set of objects $\set{B_i | i \in I}$, then $\mathscr{A}$ has an initial object.

  First form $P = \prod_{i \in I} B_i$, then $P$ is weakly initial.
  Now form the limit of
  \begin{equation*}
    \begin{tikzcd}
      P \rar[shift left] \rar[shift right] \rar & P
    \end{tikzcd} \tag{$*$}\label{eq:12star}
  \end{equation*}
  where edges are all the endomorphisms of $P$: denote the limit $I \overset{i}\to P$.
  $I$ is also weakly initial in $\mathscr{A}$: suppose given $\begin{tikzcd}I \rar[shift left]{f} \rar[shift right]{g} & C\end{tikzcd}$.
...
\begin{nexample}\label{eg:4.13}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    \item Consider the \hyperlink{def:forgFunc}{forgetful functor} $U: \gpc \to \setc$.
      By \cref{eg:4.6}(a), $U$ creates all small limits, so $\gpc$ has them and $U$ preserves them.
      $\gpc$ is \hyperlink{def:lsmall}{locally small}, given a set $A$, any $f: A \to UG$ factors as $A \to UG' \to UG$, where $G'$ is the subgroup generated by $\set{f(x) | x \in A}$ and $\operatorname{card} G' \leq \max\{\aleph_0, \operatorname{card} A\}$.
