...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    ...
    \item For any \hyperlink{def:small}{small} category $\mathscr{C}$, $\hyperlink{def:functcat}{[\mathscr{C}^{\text{op}}, \mathbf{Set}]}$ is a topos: we've seen that it's \hyperlink{def:cc}{cartesian closed}, and $\Omega$ is determined by \hyperlink{lem:yoneda}{Yoneda}:
      \begin{equation*}
