...
\begin{nthm}[Precise Monadicity Theorem]\label{thm:5.12}
  \marginnote{\emph{Lecture 17}}$G: \dc \to \cc$ is \hyperlink{def:monadic}{monadic} iff $G$ has a \hyperlink{def:adj}{left adjoint} and \hyperlink{def:clim}{creates} \hyperlink{def:equalizer}{coequalizers} of \hyperlink{def:gsplit}{$G$-split} pairs.
\end{nthm}
\begin{nthm}[Refined/Reflexive Monadicity Theorem]\label{thm:5.13}
  Suppose $\dc$ has and $G: \dc \to \cc$ preserves \hyperlink{def:reflexive}{reflexive coequalizers} and that $G$ reflects isomorphisms and has a \hyperlink{def:adj}{left adjoint}. Then $G$ is \hyperlink{def:monadic}{monadic}.
\end{nthm}
\begin{proof}[Proof of \cref{thm:5.12} $\Rightarrow$]
  It is sufficient to show that $\hyperlink{def:fsupt}{G^\mathbb{T}}: \hyperlink{def:em}{\cc^\mathbb{T}}\to\cc$ \hyperlink{def:clim}{creates} \hyperlink{def:equalizer}{coequalizers} of \hyperlink{def:gsplit}{$G^\mathbb{T}$ split}-pairs.
...
\begin{proof}[Proof of \cref{thm:5.12} $\Leftarrow$ and \cref{thm:5.13}]
  ...
  \end{tikzcdi}
  $
  is both \hyperlink{def:reflexive}{reflexive} and \hyperlink{def:gsplit}{$G$-split}, so has a \hyperlink{def:equalizer}{coequalizer} in $\dc$, and hence by \cref{lem:5.11}, $\hyperlink{def:emcomp}{K: \dc \to \cc^\mathbb{T}}$ has a \hyperlink{def:adj}{left adjoint} $L$.
...
\begin{nexample}\label{eg:5.14}\leavevmode
  \begin{enumerate}
    \item The \hyperlink{def:forgFunc}{forgetful functors} $\gpc \to \setc$, $\rngc \to \setc$, $\modc_R \to \setc, \dotsc$ all satisfy the hypotheses of \cref{thm:5.13}; for the \hyperlink{def:reflexive}{reflexive coequalizers}, use question 3 on example sheet 4 which shows that if
      \begin{equation*}
        \begin{tikzcd}
          A \rar[shift left]{f} \rar[shift right,']{g} & B \rar{h} & C
        \end{tikzcd}
      \end{equation*}
      is a reflexive coequalizer diagram in $\setc$, then so is
      \begin{equation*}
        \begin{tikzcd}
    ...
    \item Any \hyperlink{def:refl}{reflection} is \hyperlink{def:monadic}{monadic:} this follows from q2 on example sheet 3, but also can be proved using \cref{thm:5.12}.
      Let $\dc$ be a \hyperlink{def:refl}{reflective} (\hyperlink{def:fulls}{full}) subcategory of $\cc$ and suppose a pair
      $
      \begin{tikzcdi}
        A \rar[shift left]{f} \rar[shift right,']{g} & B
      \end{tikzcdi}
      $ in $\dc$ fits into a \hyperlink{def:splitcoeq}{split coequalizer} diagram
      \begin{equation*}
        \begin{tikzcd}
      ...
      \end{equation*}
      in $\cc$.
      Then $t$ and $ft=sh$ belong to $\dc$ since $\dc$ is full, and hence $s$ is in $\dc$ since it's an equalizer of $(1_B, sh)$ and $\dc$ is closed under limits in $\cc$.
      Hence also $h \in \mor \dc$.
    \item Consider the composite adjunction
      \begin{equation*}
      \begin{tikzcd}
        \setc \rar[shift left]{F} & \mathbf{AbGp} \rar[shift left]{L} \lar[shift left]{G} & \mathbf{tfAbGp}. \lar[shift left]{I}
      \end{tikzcd}
      \end{equation*}
      The two factors are monadic by (a) and (b) respectively, but the composite isn't, since the monad it induces on $\setc$ is isomorphic to that induced by $(F \dashv U)$.
    \item Consider the forgetful functor $\topc \overset{U}\to \setc$.
      This is \hyperlink{def:full}{faithful} and has both \hyperlink{def:adj}{left and right adjoints} (so preserves all \hyperlink{def:equalizer}{coequalizers}), but the monad induced on $\setc$ is $(1,1,1)$ and the category of algebras is $\setc$.
    \item Consider the composite adjunction
      \begin{equation*}
      \begin{tikzcd}
        \setc \rar[shift left]{D} & \mathbf{Top} \rar[shift left]{\beta} \lar[shift left]{U} & \mathbf{KHaus}. \lar[shift left]{I}
      \end{tikzcd}
      \end{equation*}
      ...
      \begin{equation*}
        ...
        \end{tikzcd}
      \end{equation*}
      be a split coequalizer in $\setc$, where $X,Y$ have compact Hausdorff topologies and $f,g$ are continuous.
