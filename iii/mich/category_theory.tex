...
\begin{ndef}[Equivalent category]\label{1.9}\index{category!equivalence}\index{equivalent}\index{categorical property}\hypertarget{def:equicat}
  ...
  By an \textbf{equivalence} between $\mathscr{C}$ and $\mathscr{D}$, we mean a pair of \hyperlink{def:funct}{functors} $F: \mathscr{C} \to \mathscr{D}$, $G: \mathscr{D} \to \mathscr{C}$ together with natural isomorphisms $\alpha: 1_\mathscr{C} \to GF$ and $\beta: FG \to \mathscr{D}$.
...
\index{split monomorphism}\index{monomorphism!split}\hypertarget{def:split}We call such monomorphisms \textbf{split}.
...
\begin{nlemma}\label{2.2}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item Then natural transformations $\cc(A, -) \to F$ is a bijection with elements of $FA$.
...
\begin{proof}[Proof of \cref{2.2}(i)]
    Given $\alpha: \cc(A,-) \to F$, we define
      ...
      Given $\alpha$,
      \begin{align*}
        \Psi\Phi(\alpha)_B(f) &= \Phi(\alpha_A(1_A))_B(f) = Ff(\alpha_A(1_A)) \\
...
\end{proof}
\hypertarget{def:yonemb}We call this \hyperlink{def:funct}{functor} (or the functor $\cc \to [\cc^{op}, \setc]$) sending $A$ to $\cc(-,A)$ the \textbf{Yoneda embedding} of $\cc$, and denote it by $Y$.
...
\begin{ndef}\label{2.4}\index{representable}\index{representation}\hypertarget{def:repr}
...
\begin{nexample}\label{2.6}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Let $A,B$ be two objects of a \hyperlink{def:lsmall}{locally small} \hyperlink{def:cat}{category} $\cc$.
      ...
      \hypertarget{def:catprod}A representation of this, if it exists, is called a (categorical) \textbf{product} of $A$ and $B$, and denoted
      \begin{equation*}
      ...
      Dually we have the notion of \textbf{coproduct} $(A + B, A \overset{\nu_1}\to A + B, B \overset{\nu_2}\to A + B)$.
