...
\begin{ndef}[Kleisli category]\label{def:5.5}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  (commutes by \hyperlink{eq:monad1}{(1)}) and
  \begin{equation*}
    \begin{tikzcd}[row sep=large]
    ...
    \end{tikzcd}
  \end{equation*}
  which commutes by \hyperlink{eq:monad2}{(2)}.
\end{ndef}
...
\begin{ndef}\leavevmode
  ...
  \item \hypertarget{def:splitcoeq}By a \named{split coequalizer} diagram, we mean a diagram
    ...
    \item Given a functor $G: \mathscr{D} \to \mathscr{C}$, a parallel pair $\begin{tikzcdi}A \rar[shift left]{f} \rar[shift right,']{g} & B\end{tikzcdi}$ is called $G$-\textbf{split} if there exists a split coequalizer diagram
    \begin{equation*}
      \begin{tikzcd}[column sep=large]
 ...
 Note that $\begin{tikzcd}[cramped, column sep=large]FGFGB \rar[shift left]{FG\epsilon_B} \rar[shift right,']{\epsilon_{FGB}} & FGB\end{tikzcd}$ is $G$-split, since
    \begin{equation*}
      \begin{tikzcd}[column sep=large]
