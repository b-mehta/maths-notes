...
\begin{ndef}[Category]\label{1.1}\index{category}\hypertarget{def:cat}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \index{identity}an operation assigning to each $A \in \ob \cc$ a morphism $\begin{tikzcdi}A \rar{1_A} & A\end{tikzcdi}$, called its \textbf{identity}.
...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]\label{1.5}
    ...
    \item Given a set $A$, we write $PA$ for the set of all subsets of $A$.
      We can make $P$ into a functor $\setc \to \setc$: given $\begin{tikzcdi} A \rar{f}& B \end{tikzcdi}$, we define $Pf(A') = \set{f(a) | a \in A'}$ for $A' \subseteq A$.
...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]\label{1.7}
    ...
    \item Let $A,B$ be two $G$-sets regarded as functors $G \to \setc$.
...
\begin{proof}\leavevmode
  \begin{itemize}
    ...
    \item[$\impliedby$] Suppose each $\alpha_A$ has an inverse $\beta_A$. Given $f: A \to B$ in $\mathscr{C}$, we need to show that
      \begin{equation*}
        \begin{tikzcd}
...
\begin{ndef}[Equivalent category]\label{1.9}\index{category!equivalence}\index{equivalent}\index{categorical property}\hypertarget{def:equicat}
  ...
  We say a property $P$ of \hyperlink{def:cat}{categories} is a \textbf{categorical property} if whenever $\cc$ has $P$ and $\cc \simeq \dc$, then $\dc$ has $P$.
...
\begin{nexample}\label{1.10}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    \item The category \hyperlink{def:categ}{$\partc$} is equivalent to the category $\setc_*$ of pointed sets (and basepoint-preserving functions).
      We define $F: \setc_* \to \partc$ by $F(A,a) = A \setminus \{a\}$ and if $f: (A,a) \to (B,b)$
      \begin{equation*}
        Ff(x) =
        \begin{cases}
          f(x) & \text{if } f(x) \neq b \\
      ...
      \end{equation*}
      and
      $G : \partc \to \setc_*$ by $G(A) = A^+ = A \cup \{A\}$ and if $f: A \to B$ is a partial function, we define $Gf: A^+ \to B^+$ by
      \begin{equation*}
        Gf =
        \begin{cases*}
        ...
        \end{cases*}
      \end{equation*}
      The composite $FG$ is the identity on $\partc$, but $GF$ is not the identity, however there's an isomorphism
      \begin{equation*}
        (A,a) \to ((A \setminus \{a\})^+, A \setminus \{a\})
      \end{equation*}
      sending $a$ to $A \setminus \{a\}$ and everything else to itself and this is natural.

      Note that there can be no isomorphism $\setc_* \to \partc$ since $\partc$ has a $1$-element isomorphism class $\{\emptyset\}$ and $\setc_*$ doesn't.
...
\begin{ndef}[Faithful, full, essentially surjective]\label{1.11}\hypertarget{def:full}
 ...
 \begin{enumerate}[label=(\alph*)]
   \item \index{functor!faithful}We say $F$ is \textbf{faithful} if, given $f, f' \in \mor \cc$ with $\dom f = \dom f'$, $\cod f = \cod f'$ and $Ff = Ff'$ then $f = f'$.
   ...
   \item \index{functor!essentially surjective}We say $F$ is \textbf{essentially surjective} if, for every $B \in \ob \dc$, there exists $A \in \ob \cc$ and an isomorphism $FA \to B$ in $\dc$.
 \end{enumerate}
 \hypertarget{def:fulls}We say a subcategory $\cc' \subseteq \cc$ is \textbf{full} if the inclusion $\cc' \to \cc$ is a full functor.
\end{ndef}
\begin{eg}
  $\gpc$ is a \hyperlink{def:fulls}{full subcategory} of $\monc$, but $\monc$ is not a full subcategory of the category $\sgpc$ of semigroups.
\end{eg}
\begin{nlemma}\label{1.12}
  Assuming the axiom of choice, a functor $F: \cc \to \dc$ is part of an \hyperlink{def:equicat}{equivalence} $\cc\simeq\dc$ iff it is \hyperlink{def:full}{full}, \hyperlink{def:full}{faithful} and \hyperlink{def:full}{essentially surjective}.
\end{nlemma}
\begin{proof}\leavevmode
  \begin{itemize}
    \item[$\implies$] Given $G,\alpha,\beta$ as in \cref{1.9}, for each $B\in\ob\dc$, $\beta_B$ is an isometry $FGB\to B$, so $F$ is essentially surjective.
    ...
    \item[$ \impliedby $] For each $B\in\ob\dc$, choose $ GB\in\ob\cc$ and an isomorphism $\beta_B: FGB \to B$ in $\dc$.
      Given
      \begin{equation*}
        \begin{tikzcd}
          B\rar{g} & B'
      ...
      Given $A\in\ob\cc $, define $\alpha_A: A \to GFA$ to be the unique morphism whose image under $F$ is
      \begin{equation*}
        \begin{tikzcd}
...
\begin{ndef}[Monomorphism, epimorphism]\label{1.14}
  Let $A \xrightarrow{f} B$ be a \hyperlink{def:cat}{morphism} in $\cc$
  \begin{enumerate}[label=(\alph*)]
...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    \item In $\setc$, \hyperlink{def:monic}{mono} $\iff$ injective ($\implies$ easy; for $\impliedby$ take $C = 1 = \{*\}$) and \hyperlink{def:epic}{epi} $\iff$ surjective ($\implies$ easy; for $\impliedby$ use two morphisms $B \to 2 = \{0,1\}$).
      So $\setc$ is \hyperlink{def:balanced}{balanced}.
    \item In $\gpc$ mono $\iff$ injective (for $\impliedby$ use homoms $\mathbb{Z} \to A$) and epi $\iff$ surjective ($\impliedby$ uses free products with amalgamation).
      So $\gpc$ is balanced.
    \item In $\rngc$, mono $\iff$ injective (proof much as for $\gpc$) but the inclusion $\mathbb{Z} \to \mathbb{Q}$ is an epimorphism, since if \begin{tikzcd}[cramped] \mathbb{Q} \arrow[r, shift left, "f"] \arrow[r, shift right,"g"'] & R \end{tikzcd}
      agree on all integers, they agree everywhere.
      So $\rngc$ isn't balanced.
    \item In $\topc$, mono $\iff$ injective and epi $\iff$ surjective (proofs as in $\setc$).
        But $\topc$ isn't balanced since a continuous bijection needn't have a continuous inverse.
  \end{enumerate}
\end{nexample}
...
\begin{ndef}[Locally small]\label{2.1}\index{locally small}\hypertarget{def:lsmall}
  We \marginnote{\emph{Lecture 5}}[0cm]say a \hyperlink{def:cat}{category} $\cc$ is \textbf{locally small} if, for any two objects $A,B$, the morphisms $A \to B$ in $\cc$ form a set $\cc(A,B)$.
\end{ndef}

 If we fix $A$ and let $B$ vary, the assignment $B \mapsto \cc(A,B)$ becomes a \hyperlink{def:funct}{functor} $\hyperlink{def:lsmall}{\cc(A, -)} : \cc \to \setc$: given $\begin{tikzcdi} B \rar{f} & C \end{tikzcdi}$, $\cc(A,f)$ is the mapping $g \mapsto fg$.
Similarly, $A \mapsto \cc(A,B)$ defines a functor $\cc(-, B): \cc^{op} \to \setc$.
\begin{nlemma}[Yoneda Lemma]\index{Yoneda!lemma}\label{2.2}
  Let $\cc$ be a \hyperlink{def:lsmall}{locally small} \hyperlink{def:cat}{category}, $A \in \ob \cc$ and $F: \cc \to \setc$ a \hyperlink{def:funct}{functor}.
  \begin{enumerate}[label=(\roman*)]
    \item Then natural transformations $\cc(A, -) \to F$ are in bijection with elements of $FA$.
    \item Moreover, this bijection is natural in both $A$ and $F$.
  \end{enumerate}
\end{nlemma}
\begin{proof}[Proof of \cref{2.2}(i)]
  Given $\alpha: \cc(A,-) \to F$, we define
  \begin{equation*}
    \Phi(\alpha) = \alpha_A(1_A) \in FA.
  \end{equation*}
  Given $x \in FA$, we define $\Psi(x) : \cc(A,-) \to F$ by
  \begin{equation*}\Psi(x)_B(A \overset{f}{\to} B) = Ff(x) \in FB.\end{equation*}
  $\Psi(x)$ is natural: given $g: B \to C$, we have
  \begin{align*}
    \Psi(x)_C \cc(A,g)(f) = \Psi(x)_C(gf) &= F(gf)(x) \\
  ...
  \begin{align*}
    \Psi\Phi(\alpha)_B(f) &= \Psi(\alpha_A(1_A))_B(f) = Ff(\alpha_A(1_A)) \\
                          &= \alpha_B \cc(A,f) (1_A) = \alpha_B(f)
  \end{align*}
  so $\Psi \Phi(\alpha) = \alpha$.
\end{proof}
\begin{ncor}\label{2.3}
  The assignment $A \mapsto \hyperlink{def:lsmall}{\cc(A, -)}$ defines a \hyperlink{def:full}{full} and \hyperlink{def:full}{faithful} \hyperlink{def:funct}{functor} $\cc^{op} \to \hyperlink{def:functcat}{[\cc, \setc]}$.
\end{ncor}
\begin{proof}
  Put $F = \cc(B, -)$ in \cref{2.2}(i): we get a bijection between $\cc(B,A)$ and morphisms $\cc(A, -) \to \cc(B, -)$ in $[\cc, \setc]$.
...
\end{proof}
\hypertarget{def:yonemb}We\index{Yoneda!embedding} call this \hyperlink{def:funct}{functor} (or the functor $\cc \to [\cc^{op}, \setc]$) sending $A$ to $\cc(-,A)$ the \textbf{Yoneda embedding} of $\cc$, and denote it by $Y$.

\begin{proof}[Proof of \cref{2.2}(ii)]
  Suppose for the moment that $\cc$ is \hyperlink{def:small}{small}, so that $\hyperlink{def:lsmall}{[\cc, \setc]}$ is \hyperlink{def:lsmall}{locally small}.
  Then we have two functors $\cc \times [\cc, \setc] \to \setc$:
  One sends $(A,F)$ to $FA$, and the other is the composite
  \begin{equation*}
    \begin{tikzcd}[column sep=6em]
      \cc \times [\cc, \setc] \arrow[r,"Y \times 1"] &
      \left[\cc, \setc\right]^{op} \times \left[\cc, \setc\right]
      \rar{[\cc, \setc](-, -)} & \setc
    \end{tikzcd}
  \end{equation*}
  \cref{2.2}(ii) says that these are naturally isomorphic.

  We can translate this into an elementary statement, making sense even when $\cc$ isn't small, given $A \overset{f}{\to} B$ and $F \overset{\alpha}{\to} G$, the two ways of producing an element of $GB$ from a natural transformation $\beta: \cc(A, -) \to F$ give the same result, namely
  \begin{equation*}
...
\begin{ndef}\label{2.4}\index{universal element}\index{representable}\index{representation}\hypertarget{def:repr}
  We say a \hyperlink{def:funct}{functor} $F: \cc \to \setc$ is \textbf{representable} if it's \hyperlink{def:natiso}{isomorphic} to $\cc(A, -)$ for some $A$.
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}
      \hyperlink{def:lsmall}{\cc(B, -)} \rar{\Psi(y)} &
      F \rar{\Psi(x)^{-1}} &
      \cc(A,-)
    \end{tikzcd}
  \end{equation*}
  ...
  \begin{equation*}
    \begin{tikzcd}[row sep=large]
      \cc(B, -) \arrow[rr, "Y(f)"] \arrow[rd, "\Psi(y)"'] & & \cc(A,-) \dlar{\Psi(x)} \\
      & F
    \end{tikzcd}
  \end{equation*}
  commutes iff $(Ff) x = y$.
...
\begin{nexample}\label{2.6}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    \item The \hyperlink{def:forgFunc}{forgetful functor} $\gpc \to \setc$ is \hyperlink{def:repr}{representable} by $(\mathbb{Z}, 1)$.
      Similarly, the forgetful functor $\rngc \to \setc$ is representable by $(\mathbb{Z}[x], x)$ and
      the forgetful functor $\topc \to \setc$ is representable by $(\{*\}, *)$.
    \item The functor $P^*: \setc^{op} \to \setc$ (see \cref{1.5}(c)) is representable by $(\{0,1\}, \{1\})$: this is the bijection between subsets and characteristic functions.
    \item Let $G$ be a group.
      The unique (up to \hyperlink{def:iso}{isomorphism}) representable functor $G(*, -): G \to \setc$ is the \emph{Cayley representation} of $G$, i.e.\ the set $UG$ with $G$ acting by left multiplication.
    \item Let $A,B$ be two objects of a \hyperlink{def:lsmall}{locally small} \hyperlink{def:cat}{category} $\cc$.
      We have a functor $\cc^{op} \to \setc$ sending $C$ to $\cc(C,A) \times \cc(C,B)$.
      ...
      Products exist in many categories of interest: in $\setc, \gpc, \rngc, \topc$ they are `just' Cartesian products, in posets they are binary meets.
    ...
    \item \marginnote{\emph{Lecture 6}}[0cm] The dual-vector-space functor $\modc_K^{op} \to \modc_K$, when composed with the forgetful functor $\modc_K \to \setc$, is representable by $(K, 1_K$).
    \item Let \begin{tikzcd}[cramped] A \arrow[r, shift left, "f"] \arrow[r, shift right,"g"'] & B \end{tikzcd} be morphisms in a locally small category $\cc$.
      We have a functor $F: \cc^{op} \to \setc$ defined by
      \begin{equation*}
        F(C) = \set{h \in \cc(C, A) | fh = gh}.
      \end{equation*}
      ...
      In $\setc$, we take $E = \set{x \in A | f(x) = g(x)}$ and $e = $ inclusion.

      Similar constructions work in $\gpc, \rngc, \topc, \dotsc$.

      Dually, we have the notion of \textbf{coequalizer}.
  \end{enumerate}
\end{nexample}

\begin{nremark}\label{2.7}
...
\begin{ndef}[Separating, detecting families]\label{def:2.8}
  Let $\cc$ be a category, $\mathscr{G}$ a class of objects of $\cc$.
  \begin{enumerate}[label=(\alph*)]
    \item \index{separating family}\hypertarget{def:separating}We say $\mathscr{G}$ is a \textbf{separating family} for $\cc$ if, given \begin{tikzcd}[cramped] A \arrow[r, shift left, "f"] \arrow[r, shift right,"g"'] & B \end{tikzcd} such that $fh=gh$ for all $G \overset{h}{\to} A$ with $G \in \mathscr{G}$, then $f = g$. (i.e.\ the functors $\cc(G,-)$, $G \in \mathscr{G}$ are collectively faithful).
    \item \index{detecting family}\hypertarget{def:detecting}We say $\mathscr{G}$ is a \textbf{detecting family} for $\cc$ if, given $A \overset{f}\to B$ such that every $G \overset{h}{\to} b$ with $G \in \mathscr{G}$ factors uniquely through $f$, then $f$ is an isomorphism.
  \end{enumerate}
  If $\mathscr{G} = \{G\}$, we call $G$ a \textbf{separator}/\textbf{detector}.
\end{ndef}

\begin{nlemma}\label{lem:2.9}
  \begin{enumerate}[label=(\roman*)]
    \item If $\cc$ is a \hyperlink{def:balanced}{balanced} category, then any \hyperlink{def:separating}{separating} family is \hyperlink{def:detecting}{detecting}.
    \item If $\cc$ has \hyperlink{def:equalizer}{equalizers}, then any detecting family is separating.
  \end{enumerate}
\end{nlemma}
\begin{proof}
  \begin{enumerate}[label=(\roman*)]
...
\begin{nexample}\label{eg:2.10}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    \item In $[\cc, \setc]$ the family
      \begin{equation*}
        \set{\cc(A, -) | A \in \ob \cc}
      \end{equation*}
      is both \hyperlink{def:separating}{separating} and \hyperlink{def:detecting}{detecting} (this is just a restatement of \nameref{2.2}.)
    \item In $\setc$, $1 = \{*\}$ is both a separator and a detector since it represents the identity \hyperlink{def:funct}{functor} $\setc \to \setc$.

      Similarly, $\mathbb{Z}$ is both in $\gpc$, since it represents the forgetful functor $\gpc \to \setc$.

      And $2 = \{0,1\}$ is a coseparator and a codetector in $\setc$, since it represents $P^*: \setc^{op} \to \setc$.
    \item In $\topc$, $1 = \{*\}$ is a separator since it represents the forgetful functor $\topc \to \setc$, but not a detector.
      In fact, $\topc$ has no detecting \emph{set} of objects.
    ...
    \item Let $\cc$ be the category of pointed connected CW-complexes and homotopy classes of (basepoint-preserving) continuous maps.
      JHC Whitehead proved that if $X \overset{f}\to Y$ in this category induces isomorphisms $\pi_n(X) \to \pi_n(Y)$ for all $n$, then it's an isomorphism in $\cc$.
      This says that $\set{S^n | n \geq 1}$ is a detecting set for $\cc$.

      But PJ Freyd showed there is no faithful functor $\cc \to \setc$, so no separating \emph{set}: if $\set{G_i | i \in I}$ were separating, then
      \begin{equation*}
        x \mapsto \coprod_{i \in I} \cc(G_i, X)
      \end{equation*}
      would be faithful.
  \end{enumerate}
\end{nexample}
Note that any functor of the form $\cc(A, -)$ preserves monos, but they don't normally preserve epis.
...
\begin{ndef}
  ...
  (If $\cc$ is \hyperlink{def:lsmall}{locally small}, this says $\cc(P, -)$ preserves epis).

  Dually, an \textbf{injective} object of $\cc$ is a projective object of $\cc^{op}$.
...
\end{ndef}
\begin{nlemma}
  Representable functors are (pointwise) projective in $[\cc, \setc]$.
  \begin{center}
    \begin{tikzcd}
...
F \arrow[r, "a", two heads] & G
  ...
  By Yoneda, $\beta$ corresponds to some $y \in GA$, and we can find $x \in FA$ with $\alpha_A(x) = y$.
  Now if $\gamma: \cc(A, -) \to F$ corresponds to $x$ then naturality of $x$ %finish
\end{nlemma}
\printindex
\end{document}
