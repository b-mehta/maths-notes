...
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      P \rar[shift left=0.5] \rar[shift left=2] \rar[shift left=3.5] \rar[shift right=2] & P
...
\begin{nthm}[Special adjoint functor theorem]\label{thm:4.16saft}
  ...
  Then a \hyperlink{def:funct}{functor} $G:\mathscr{D}\to\mathscr{C}$ has a \hyperlink{def:adj}{left adjoint} iff it is \hyperlink{def:plim}{preserves} all \hyperlink{def:limit}{small limits}.
\end{nthm}
\begin{proof}
...
\begin{nexample}\label{eg:4.17}
  Consider the inclusion $\textbf{KHaus} \overset{I}\to \textbf{Top}$, where $\textbf{KHaus}$ is the \hyperlink{def:fulls}{full subcategory} of compact Hausdorff spaces.
  $\textbf{KHaus}$ \hyperlink{def:haslimits}{has} and $I$ \hyperlink{def:plim}{preserves} \hyperlink{def:small}{small} \hyperlink{def:lprod}{products} (by Tychonoff's Theorem) and \hyperlink{def:equalizer}{equalizers} (since equalizers of pairs $\begin{tikzcdi}X \rar[shift left]{f} \rar[shift right,']{g} & Y\end{tikzcdi}$ with $Y$ Hausdorff are closed subspaces).
  Both categories are \hyperlink{def:lsmall}{locally small} and \textbf{KHaus} is \hyperlink{def:wp}{well-powered} (subobjects of $X$ are all isomorphic to closed subspaces).
...
\begin{ndef}[Monad]\hypertarget{def:monad}
  A \named{monad} $\mathbb{T} = (T, \eta, \mu)$ on a \hyperlink{def:cat}{category} $\mathscr{C}$ consists of a \hyperlink{def:funct}{functor} $T: \mathscr{C} \to \mathscr{C}$ and \hyperlink{def:nattrans}{natural transformation} $\eta: 1_\mathscr{C} \to T$, $\mu: TT \to T$ satisfying equations \eqref{eq:monad1}-\eqref{eq:monad3}.
...
This adjunction gives rise to the monad of \cref{eg:5.2}(b).
...
\begin{ndef}[Eilenberg-Moore category]\label{def:5.3}
  ...
  A $\mathbb{T}$-\named{algebra} is a pair $(A, \alpha)$ with $A \in \ob \mathscr{C}$ and $TA \overset{\alpha}\to A$ satisfying the commutative diagrams
  \begin{equation}
    \begin{tikzcd}
      A \rar{\eta_A} \drar[']{1_A} & TA \dar{\alpha} \\ & A
    \end{tikzcd} \tag{4} \label{eq:em4}
  \end{equation}
  \begin{equation}
    \begin{tikzcd}
      TTA \rar{T\alpha} \dar{\mu_A} & TA \dar{\alpha} \\ TA \rar{\alpha} & A.
    \end{tikzcd} \tag{5} \label{eq:em5}
  \end{equation}
  A \textbf{homomorphism}\label{algebra!homomorphism} $f: (A, \alpha) \to (B, \beta)$ is a morphism $A \overset{f}\to B$ such that
  \begin{equation}
  \begin{tikzcd}
    TA \rar{Tf} \dar{\alpha} & TB \dar{\beta} \\ A \rar{f} & B \tag{6} \label{eq:em6}
  \end{tikzcd}
  \end{equation}
  commutes.
...
\begin{nlemma}\label{lem:5.4}
  The \hyperlink{def:forgFunc}{forgetful functor} $G^\mathbb{T}: \mathscr{C}^\mathbb{T}  \to \mathscr{C}$ has a \hyperlink{def:adj}{left adjoint} $F^\mathbb{T}$ and the adjunction induces $\mathbb{T}$.
\end{nlemma}
\begin{proof}
  We define $F^\mathbb{T} A = (TA, \mu_A)$ (an \hyperlink{def:em}{algebra} by \eqref{eq:monad2} and \eqref{eq:monad3}) and $F^\mathbb{T}(A \overset{f}\to B) = Tf$ (a \hyperlink{def:em}{homomorphism} by \hyperlink{def:nattrans}{naturality} of $\mu$).
  ...
  (a homomorphism by \eqref{eq:em5})
  $\epsilon$ is natural by \eqref{eq:em6}; for the triangular identities, $\epsilon_{FA} (F \eta_A) = 1_{FA}$ is \eqref{eq:monad1}, $G \epsilon_{(A,\epsilon)} \eta_A = 1_A$ is \eqref{eq:em4}.
