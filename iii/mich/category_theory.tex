...
\begin{proof}\leavevmode
  \begin{itemize}
    ...
    \item [$\Rightarrow$] If $B$ is \hyperlink{def:exp}{exponentiable}, for any $A \overset{f}\to B$ we define $\Pi_B(f)$ to be the \hyperlink{def:pullback}{pullback}
  \begin{equation*}
  \begin{tikzcd}
...
\begin{nexample}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    ...
    \item For any \hyperlink{def:small}{small} category $\mathscr{C}$, $[\mathscr{C}, \mathbf{Set}]$ is \hyperlink{def:cc}{cartesian closed}: by Yoneda,
      ...
      \end{align*}
      so we take the right hand side as a definiition of $G^F(A)$ and define $G^F$ on morphisms $A \overset{f}\to B$ by composition with $\mathscr{C}(f,-) \times 1_F$.
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  Since $fa = fb$, $f$ factors as $A \overset{q}\to I \overset{m}\to B$.
  ...
  Now $fk=mqk=mgn=mhn=mql=fl$, so $\exists E \overset{p}\to R$ with $ap = k$, $bp=l$.
  Now $qk=qap=qbp=ql$, i.e.\ $gn=hn$. But $n$ is epic, so $g=h$.
  Hence $m$ is monic.
\end{proof}
Note that this implies any strong epimorphism $A \overset{f}\to B$ is \hyperlink{def:regular}{regular}, since the \hyperlink{def:monic}{monic} part of its image factorization is an \hyperlink{def:iso}{isomorphism}.
...
\end{ndef}
Note that the second part of \cref{lem:6.4} says that if $\mathscr{C}$ is \hyperlink{def:lcc}{locally cartesian closed} then $\hyperlink{def:bstar}{f^*}: \mathscr{C}/B \to \mathscr{C}/A$ is a \hyperlink{def:ccfunct}{cartesian closed functor} for any $A \overset{f}\to B$.
  ...
  \begin{itemize}
    \item[($\Rightarrow$)] \marginnote{\emph{Lecture 20}} Given an inverse for $\theta: F(C^A) \to FC^{FA}$, we define $\varphi_{A,B}^{-1}$ to be the composite
      \begin{equation*}
        ...
        LB \times A \arrow[r, "L\lambda \times 1"] & L((B \times FA)^{FA}) \arrow[r, "L(\eta^{FA}) \times 1"] & L(FL(B \times FA)^{FA}) \times A \arrow[d, "L\theta^{-1} \times 1"] \\
...
\begin{proof}\leavevmode
  \begin{itemize}
    \item[($\Rightarrow$)] $L$ \hyperlink{def:plim}{preserves} \hyperlink{def:terminal}{$1$}, so if we substitute $B=1$ in the definition of $\varphi$, we get
      \begin{equation*}LFA \overset{\epsilon_A} \to A.\end{equation*}
...
\begin{nexample}\leavevmode\label{eg:6.12}
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \hypertarget{def:presheaf}Let $X$ be a topological space. By a \named{presheaf} on $X$, we mean a \hyperlink{def:funct}{functor}
      \begin{equation*}\mathcal{O}(X)^{\text{op}} \overset{F}\to \mathbf{Set},\end{equation*}
...
\begin{ndef}[Subobject classifier, topos]\label{def:7.1}\leavevmode
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \hypertarget{def:topos}A \index{topos} is a category which has finite limits, is \hyperlink{def:cc}{cartesian closed} and has a subobject classifier.
...
\end{nlemma}
\begin{proof}
  Given $A \overset{f}\to B$, we define $PB \overset{Pf}\to PA$ to correspond to the \hyperlink{def:pullback}{pullback}
  \begin{equation*}
  \begin{tikzcd}
    E_f \rar \dar[tail] & \ni_B \dar[tail]\\ PB\times A \rar{1 \times f} & PB \times B.
  \end{tikzcd}
  \end{equation*}
  For any $C \overset{\hyperlink{def:power}{\ceil{m}}}\to PB$, it's easy to see that $(Pf) \ceil{m}$ corresponds to $(1_C \times f)^* (m)$; hence $f \mapsto Pf$ is \hyperlink{def:funct}{functorial}.
...
\begin{nlemma}\label{lem:7.6}
  Given $A \overset{f}\to B$, $\hyperlink{def:brace}{\{\}_B} f$ corresponds to $\begin{tikzcd}[cramped]A \rar[tail]{(1_A, f)} & A \times B\end{tikzcd}$ and $(\hyperlink{def:power}{Pf})\{\}_B$ corresponds to $\begin{tikzcd}[cramped]A \rar[tail]{(f, 1_A)} & B \times A\end{tikzcd}$.
\end{nlemma}
\begin{proof}
  The square
  \begin{equation*}
...
\begin{proof}\leavevmode
  ...
  \end{enumerate}
\end{proof}
\hypertarget{def:exists}Given a \hyperlink{def:monic}{monomorphism} $A \overset{f}\to B$ in $\mathscr{E}$, we define $\exists f: \hyperlink{def:power}{PA} \to PB$ to correspond to the composite
\begin{equation*}
\begin{tikzcd}
  \ni_A \rar[tail] & PA \times A \rar[tail]{1 \times f} &PA \times B
\end{tikzcd}
\end{equation*}
Then, for any $C \overset{\hyperlink{def:power}{\ceil{m}}} \to PA$, $(\exists f) \ceil{m}$ corresponds to
\begin{equation*}
\begin{tikzcd}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  is a \hyperlink{def:pullback}{pullback}, and since $L$ \hyperlink{def:plim}{preserves} pullbacks we deduce $Lc(A') \cong LA'$ is $\Sub(LA)$.
...
\end{proof}
\begin{ndef}[Local operator]
  Let $\mathscr{E}$ be a \hyperlink{def:topos}{topos}.
  By a \bonusnamed{local operator} on $\mathscr{E}$, we mean a morphism $j: \Omega \to \Omega$ satsifying the commutative diagrams
...
Given a closure operator on \hyperlink{def:subobj}{subobjects} as in \cref{lem:7.15}, define $\begin{tikzcdi} J \rar[tail] & \Omega \end{tikzcdi}$ to be the closure of $1 \overset{T}\rightarrowtail \Omega$ and $j: \Omega \to \Omega$ to be the classifying map of $\begin{tikzcdi} J \rar[tail] & \Omega \end{tikzcdi}$.
