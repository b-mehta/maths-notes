...
\begin{ndef}[Category]\label{def:1.1}\index{category}\hypertarget{def:cat}
  ...
  \begin{enumerate}[label=(\alph*)]
    \item \index{object}a collection $\ob \mathscr{C}$ of \textbf{objects} $A,B,C,\dotsc$
...
\begin{nexample}\leavevmode
  ...
  \item Given a set $A$, we write $\mathcal{P}A$ for the set of all subsets of $A$.
    ...
    \item The construction of a fundamental group $\pi_1(X,x)$ of a space $X$ with basepoint $x$ is a functor $\mathbf{Top}_* \to \mathbf{Gp}$ where $\mathbf{Top}_*$ is the category of spaces with a chosen basepoint.
