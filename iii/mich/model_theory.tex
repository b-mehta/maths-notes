...
\begin{ndef}[Embedding]\label{def:1.5}\index{embedding}\hypertarget{def:embedding}
  ...
  \end{enumerate}
  \hypertarget{def:iso}An \textbf{isomorphism} of $\mathcal{M}$ into $\mathcal{N}$ is a surjective embedding (onto).
\end{ndef}
...
\index{sentence}\hypertarget{def:sentence}A \textbf{sentence} is a \hyperlink{def:form}{formula} with no \hyperlink{def:free}{free} variables. $\exists x_1 \exists x_2 \; (x_1 \cdot x_2 = 1)$ is an $L_{\text{gp}}$-sentence.
...
\begin{ndef}[$L$-theory]\label{def:3.1}\index{L-theory@$L$-theory}\hypertarget{def:ltheory}
  ...
  \hypertarget{def:model}$\mathcal{M}$ is a \textbf{model} of $T$ if $\mathcal{M} \hyperlink{def:models}{\models} \sigma$ for all $\sigma \in T$. We write $\mathcal{M} \models T$.
...
\begin{ndef}[Elementarily equivalent]\index{elementarily equivalent}\label{def:3.3}
  \hypertarget{def:eleq}Say $\mathcal{M}$ and $\mathcal{N}$ are \textbf{elementarily equivalent} if $\Th(\mathcal{M}) = \Th(\mathcal{N})$.
...
\begin{ndef}[Elementary substructure]\label{def:3.4}\leavevmode
  \begin{enumerate}[label=(\roman*)]
    \item \index{elementary embedding}\hypertarget{def:el}an \hyperlink{def:embedding}{embedding} $\beta: \mathcal{M} \to \mathcal{N}$ is \textbf{elementary} if for all \hyperlink{def:form}{formulas} $\phi(\bar{x})$ and $\bar{a} \in M^{|\bar{x}|}$,
      \begin{equation*}
        \mathcal{M} \hyperlink{def:models}{\models} \phi(\bar{a}) \iff \mathcal{N} \models \phi(\beta(\bar{a}))
      \end{equation*}
    \item \index{substructure}\hypertarget{def:subs}if $M \subseteq N$ and $\operatorname{id}: \mathcal{M} \to \mathcal{N}$ is an embedding, then $\mathcal{M}$ is said to be a \textbf{substructure} of $\mathcal{N}$, written $\mathcal{M} \subseteq \mathcal{N}$.
    \item \index{elementary substructure}\hypertarget{def:elsubs}if $M \subseteq N$ and $\operatorname{id}: \mathcal{M} \to \mathcal{N}$ is an elementary embedding, then $\mathcal{M}$ is said to be an \textbf{elementary substructure} of $\mathcal{N}$, written $\mathcal{M} \preccurlyeq \mathcal{N}$.
  \end{enumerate}
\end{ndef}
...
\begin{ndef}[Dense linear orders]\label{def:4.1}\hypertarget{def:tlo}
  A \textbf{linear order}\marginnote{\emph{Lecture 5}}[0cm]  is an $L_{\text{lo}} = \{<\}$-\hyperlink{def:lstr}{structure} such that
  \begin{enumerate}[label=(\roman*)]
    \item $\forall x \; \lnot (x < x)$
    \item $\forall x y z ((x < y \land y < z) \to x < z)$
    \item $\forall x y ((x < y) \land (y < x) \lor (x = y))$.
  \end{enumerate}
  A linear order is \textbf{dense} if it also satisfies
...
\begin{ndef}[(Finite) Partial embedding]\label{def:4.2}\index{partial embedding}\hypertarget{def:pe}
  If $\mathcal{M}, \mathcal{N} \models T_{\text{lo}}$, then an injective map $p: A \subseteq M \to N$ is a \textbf{partial embedding} if
  \begin{equation*}\mathcal{M} \models a < b \implies \mathcal{N} \models p(a) < p(b).\end{equation*}
  \index{partial embedding!finite}If $|\dom(p)| < \omega$, then $p$ is a \textbf{finite partial embedding}.
\end{ndef}
...
\begin{ndef}[Consistent, complete]\label{def:4.5}
  \index{consistent}\hypertarget{def:consistent}An $L$-theory is \textbf{consistent} if there is $\mathcal{M}$ such that $\mathcal{M} \models T$.
  If $T$ is a theory in $L$ and $\phi$ is an $L$-sentence, then $T \vdash \phi$ if for all $\mathcal{M}$ such that $\mathcal{M} \models T$, also $\mathcal{M} \models \phi$.
  \index{complete}\hypertarget{def:complete}An $L$-theory $T$ is \textbf{complete} if for all $L$-sentences $\phi$, either $T \vdash \phi$ or $T \vdash \lnot \phi$.
\end{ndef}
...
\begin{ndef}[$\omega$-categorical]\label{def:4.6}\index{categorical@$\omega$-categorical}\hypertarget{def:wcat}
  A \marginnote{\emph{Lecture 6}}[0cm] \hyperlink{def:ltheory}{theory} $T$ in a countable \hyperlink{def:lang}{language} with a countably infinite \hyperlink{def:model}{model} is \hyperlink{def:wcat}{$\omega$-categorical} if any two countable models of $T$ are isomorphic.
\end{ndef}
...
\begin{nthm}\label{thm:4.8}
  If $T$ is an \hyperlink{def:wcat}{$\omega$-categorical} theory in a countable \hyperlink{def:lang}{language}, then $T$ is \hyperlink{def:complete}{complete}.
\end{nthm}
\begin{proof}
...
\end{nremark}
\begin{proof}
  $(\implies)$ If $f_0 \subseteq f$ is a finite restriction that is not \hyperlink{def:elmap}{elementary}, then for some $\phi(\bar{x})$, $\bar{a} \in \dom(f_0)$, $\mathcal{M} \models \phi(\bar{a}) \centernot\iff \mathcal{N} \models \phi(f_0(\bar{a}))$.
...
\begin{ndef}[Random graph]\label{def:4.14}
  ...
  An $L_{\text{gph}}$-\hyperlink{def:lstr}{structure} is a \textbf{graph} if
  \begin{enumerate}[label=(\roman*)]
    \item $\forall x \; \lnot R(x,x)$
    \item $\forall xy \; (R(x,y) \leftrightarrow R(y,x))$
  \end{enumerate}

  \index{random graph}\hypertarget{def:rgraph}An $L_{\text{gph}}$-\hyperlink{def:lstr}{structure} is a \textbf{random graph} if it is a graph such that, for all $n \in \omega$,
...
\begin{ndef}[Graph theories, partial embedding]\label{def:4.16}\hypertarget{def:gpe}
  ...
  If $\mathcal{M}, \mathcal{N} \models T_{\text{gph}}$, a \textbf{partial embedding} is an injective map $p: A \subseteq M$ to $N$ such that
  \begin{equation*}
    \mathcal{M} \models R(a,b) \iff \mathcal{N} \models R(p(a), p(b))
  \end{equation*}
  for all $a,b$ in the domain.
\end{ndef}
\begin{nlemma}\label{def:4.17}
