...
\begin{ndef}\label{def:2.2}\hypertarget{def:funcAssign}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item If $t = f(t(x_1, \dotsc, x_k), \dotsc, t_{m_f}(x_1, \dotsc, x_k))$, then
      \begin{equation*}
...
\hypertarget{def:sentence}A \named{sentence} is a \hyperlink{def:form}{formula} with no \hyperlink{def:free}{free} variables. $\exists x_1 \exists x_2 \; (x_1 \cdot x_2 = 1)$ is an \hyperlink{def:lgp}{$L_{\text{gp}}$}-sentence.
...
\begin{ndef}[$L$-theory]\label{def:3.1}\index{theory}\hypertarget{def:ltheory}
  ...
  \begin{equation*}
    \Th(\mathcal{M}) = \set{\sigma | \sigma \text{ is an } L\text{-\hyperlink{def:str}{structure} and } \mathcal{M} \models \sigma}.
  \end{equation*}
\end{ndef}
...
\begin{ndef}[Elementary substructure]\label{def:3.4}\leavevmode
  \begin{enumerate}[label=(\roman*)]
    \item \index{elementary embedding, elementary map}\hypertarget{def:el}an \hyperlink{def:embedding}{embedding} $\beta: \mathcal{M} \to \mathcal{N}$ is \named{elementary} if for all \hyperlink{def:form}{formulas} $\phi(\bar{x})$ and $\bar{a} \in M^{|\bar{x}|}$,
      \begin{equation*}
...
\begin{ndef}[Consistent, complete, $\vdash$]\label{def:4.5}
  ...
  If $T$ is a \hyperlink{def:ltheory}{theory} in $L$ and $\phi$ is an \hyperlink{def:sentence}{$L$-sentence}, then we write $T \vdash \phi$ if for all $\mathcal{M}$ such that $\mathcal{M} \models T$, we also have $\mathcal{M} \models \phi$.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If $f \in \mathscr{F}$, and $\bar{c} \in \mathscr{C}^{n_R}$, then $f \bar{c} = d \in T$ for some $d \in \mathscr{C}$.
      (This is because $\exists x \; (f(\bar{c}) = x) \in T$ by \hyperlink{def:maximal}{maximality}, then apply \hyperlink{def:wp}{witness property}.)
...
Given a \hyperlink{def:complete}{complete theory} $T$ with an infinite \hyperlink{def:model}{model},
...
\begin{ndef}[Terminology and conventions]\label{def:7.1}
  ...
  \begin{itemize}
    ...
    \item A cardinality $\lambda$ is \textbf{small} if $\lambda< |U|$ (usually denote $|U|$ by $\kappa$)
...
%  \marginnote{\emph{Lecture 13}}An injective map $p: A \subseteq \mathcal{M} \to \mathcal{N}$ is a \textbf{partial embedding} if for all tuples in $A = \dom(p)$, $p$ satisfies conditions (i), (ii), (iii) in \cref{def:1.5}.
%  Idea: a partial embedding preserves quantifier-free formulas.
%\end{ndef}
%\begin{nprop}\label{prop:7.7}
%  Let $\varphi(\bar{x})$  be an $L$-formula. The following are equivalent:
%  \begin{enumerate}[label=(\roman*)]
%    \item there is $\psi(\bar{x})$, a quantifier-free $L$-formula such that
%      \begin{equation*}
%        U \models \forall x\; [\varphi(\bar{x}) \leftrightarrow \psi(\bar{x})].
%      \end{equation*}
%    \item for all partial embeddings $p: U \to U$, for all $\bar{a}$ from $\dom(\bar{p})$,
%      \begin{equation*}
...
%  (ii) $\Rightarrow$ (i). For $\bar{a} \in U$,
%  \begin{equation*}\operatorname{qftp}\set{\psi(\bar{x}) | \psi(\bar{a}) \text{ and } \psi(\bar{x}) \text{ is quantifier free})}\end{equation*}.
%  Let
%  \begin{equation*}
%  D = \set{q(\bar{x}) | q(\bar{x}) = qftp(\bar{a}) \text{ for some }\bar{a} \text{ such that } \phi(\bar{a})}.
%  \end{equation*}
%  Claim: $\phi(U) = \bigcup_{q(\bar{x}) \in D} q(U)$.
%
%  By (an argument similar to) \cref{fact:7.2}, there is $\theta_q(\bar{x})$, in $q(\bar(x)$ a finite conjunction of formulas such that $\theta_q(\bar{x}) \to \phi(x)$.
%  So we have
%  \begin{equation*}
...
%    \item every $p: U \to U$ partial embedding is elementary
%    \item If $p: U \to U$ is partial embedding and $|\dom p| < |U|$ and $b \in U$, then there is a partial embedding $\hat{p} \supseteq p$ such that $b \in \dom \hat{p}$.
%  \end{enumerate}
%\end{nthm}
%\begin{proof}
%  (i) $\Leftrightarrow$ (ii). Follows from \cref{prop:7.7}.
%
%  (ii) $\Rightarrow$ (iii). If $p: \mathcal{U} \to \mathcal{U}$ is partial embedding, then it is elementary. Let $b \in \mathcal{U}$.
%  By homogeneity of $\mathcal{U}$, there is $\alpha \in \Aut(\mathcal{U})$ such that $p \subseteq \alpha$, and so $p \cup \{\langle b, \alpha(b) \rangle \}$ is the required extension of $p$.
%
...
%\end{remark}
%
%This gives quantifier elimination for $T_{\text{rg}}$ and $T_{\text{dlo}}$.
%\begin{remark}
%  if $T$ has quantifier elimination and $\mathcal{M} \models T$, any substructure of $\mathcal{M}$ is an elementary substructure ($T$ is `model-complete').
%\end{remark}
%\begin{ndef}
%  An element $a \in \mathcal{U}$ is \textbf{definable} over $A \subseteq U$ if there is an $L(A)$-formula $\varphi(x)$ such that $\varphi(U) = \{a\}$. (In particular, any element of $A$ is definable over $A$; $x=a$ for $a \in A$).
%
%  An element $a \in U$ is \textbf{algebraic} over $A \subseteq U$ if there is an $L(A)$-formula $\varphi(x)$ such that $|\varphi(U)| < \omega$ and $a \in \phi(U)$.
%
%  The definable closure of $A$ is
%  \begin{equation*}
%    \dcl(A) = \set{a \in U | a \text{ definable over }A}
%  \end{equation*}
%  and the algebraic closure of $A$ is
%  \begin{equation*}
%    \acl(A) = \set{a \in U | a \text{ algebraic over }A}.
%  \end{equation*}
%\end{ndef}
%\begin{nprop}\label{prop:7.11}
%  For $a \in U$ and $A \subseteq U$, the following are equivalent
%  \begin{enumerate}[label=(\roman*)]
%    \item $a \in \dcl(A)$
%    \item $O(a/A) = \{a\}$. % Orb(a/A)
%  \end{enumerate}
%\end{nprop}
%\begin{proof}
%  $a \in \dcl(A)$ iff there is $\varphi(x) \in L(A)$ such that $\varphi(U) = \{a\}$. By \cref{prop:7.5} this is equivalent to invariance under $\Aut(U/A)$.
%\end{proof}
...
%    \item $a \in \acl(A)$
%    \item $|O(a/A)| < \omega$
%    \item $a \in \mathcal{M}$ for any model $\mathcal{M}$ which contains $A$.
%  \end{enumerate}
%\end{nthm}
\printindex
\end{document}
