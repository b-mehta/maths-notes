...
\begin{nlemma}[Extension lemma for dense linear orders]\label{lem:4.4}
  \index{extension lemma}Suppose $\mathcal{M} \hyperlink{def:models}{\models} \hyperlink{def:tlo}{T_{\text{lo}}}$, $\mathcal{N} \models \hyperlink{def:tlo}{T_{\text{dlo}}}$, let $p: \dom p \subseteq M \to N$ be a \hyperlink{def:pe}{finite partial embedding}.
...
\begin{ndef}[(Partial) elementary map]\label{def:4.10}\index{elementary map}\hypertarget{def:elmap}
  If $\mathcal{M}$, $\mathcal{N}$ are \hyperlink{def:str}{$L$-structures}, an injective map $f$ such that $\dom f \subseteq M$ and $\img f \subseteq N$ is called a \textbf{(partial) elementary map} if for all \hyperlink{def:form}{$L$-formulae} $\varphi(\bar{x})$ and $\bar{a} \in (\dom f)^{|\bar{x}|}$, then
  \begin{equation*}
...
A subset $S \subseteq 2^I$ has the \index{finite intersection property}\named{finite intersection property} of for all $n \in \omega$ and $A_1, \dots, A_n \in S$, it holds that,
\[
\bigcap_{i=1}^n A_i \neq \emptyset.
\] 
\end{ndef}
...
\begin{nremark}\label{rem:6.3}\leavevmode
  \begin{enumerate}[label=(\roman*)]
    \item $\hyperlink{def:tp}{\tp_{\mathcal{M}}(\bar{b}/A)}$ is \hyperlink{def:complete}{complete}, i.e.\ for every $L(A)$-\hyperlink{def:form}{formula} $\varphi(\bar{x})$, either $\varphi(\bar{x}) \in \tp(\bar{b}/A)$ or $\lnot \varphi(x) \in \tp(\bar{b}/A)$.
...
\begin{proof}
  ...
  Let $\bar{a}$ be an enumeration of $\dom(f)$, so $|\bar{a}| < \lambda$. Let $p(x, \bar{a}) \coloneqq \hyperlink{def:tp}{\tp_\mathcal{M}}(b/\bar{a}).$
  \begin{center}
    \begin{tikzpicture}[scale=2]
...
\begin{ndef}[Universality, homogeneity]\label{def:6.9}\leavevmode
  \begin{enumerate}[label=(\roman*)]
    \item \hypertarget{def:univ}An \hyperlink{def:str}{$L$-structure} $\mathcal{N}$ is \index{universal}\textbf{$\lambda$-universal} if for every \hyperlink{def:eleq}{$\mathcal{M} \equiv \mathcal{N}$} such that $|\mathcal{M}| \leq \lambda$ there is an \hyperlink{def:el}{elementary embedding} $\beta: \mathcal{M} \to \mathcal{N}$. 
  ...
  \end{enumerate}
  An $L$-structure $\mathcal{N}$ is said to be \textbf{homogeneous} if it is $\abs{\mathcal{N}}$-homogeneous. $\mathcal{N}$ is \textbf{universal} if it is $|\mathcal{N}|$-universal.
\end{ndef}
\begin{remark}
...
\begin{proof}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Then the map $f \circ \beta^{-1}: \beta(\dom(f)) \to \img(f)$ is as composition of elementary maps elementary.
...
\begin{proof}
  ...
  \item $|\varphi(\mathcal{U}, \bar{a})| < \omega$. Then $|\varphi(\mathcal{U}, f(\bar{a}))| < \omega$.
    ...
    \end{equation*}
  \item $|\varphi(U, \bar{a})| \geq \omega$. Then by strong minimality, $|\neg \varphi(\mathcal{U}, \bar{a})| < \omega$, and so
    \begin{equation*}
...
\item $\varphi(\bar{x})$ is consistent.
\end{enumerate}
...
A model $\M$ is said to \named{omit} $p(\bar x)$ if $p(\bar x)$ is not realized in $\M$. \index{model!omits}
\end{ndef}

\begin{nremark} \label{rem:9.2}
If $\M$ is a model and $p(\bar x)$ is a type in $L(M)$, then $\M$ realises $p(\bar x)$ if and only if $\M$ isolates $p(\bar x)$.
\end{nremark}

\begin{proof}
\begin{itemize}
\item[$\Leftarrow$] Let $\varphi(\bar x)$ isolate $p(\bar x)$, so $\M \models \varphi(\bar a)$ for some $\bar{a}$, so $\bar{a} \models p(\bar x)$.
\item[$\Rightarrow$] Let $\bar a \models p(\bar x)$. Then $\varphi(\bar x) \coloneqq (\bar x = \bar a)$ isolates $p(\bar x)$.
\end{itemize}
\end{proof}

\begin{nlemma} \label{lemma:9.3}
Let $\abs{L(A)} = \omega$. Let $p(\bar{x})$ be a type in $L(A)$ and suppose that $A$ does not isolate $p(\bar x)$. Let $\psi(z)$ be a consistent $L(A)$-formula in one free variable. Then there is $a \in U$ sucht that
\begin{enumerate}[label=(\roman*)]
\item $\U \models \psi(a)$,
...
\begin{nthm}[Omitting Types] \label{thm:9.4}
...
\item $A$ isolates $p(\bar x)$. \label{it:thm:9.4b}
\end{enumerate}
\end{nthm}
\begin{proof}
...
\ref{it:thm:9.4a} implies \ref{it:thm:9.4b}: Argue by contrapositon and assume $A$ does not isolate $p(\bar x)$. We build a chain $\left< A_i : i < \omega \right>$ such that
\begin{enumerate}[label=(\roman*)]
\item $A_0 = A$,
\item $\abs{A_i} = \omega$ for all $i < \omega$,
\item $A_i$ does not isolate $p(\bar{x})$ for any $i < \omega$.
\end{enumerate}
...
\end{enumerate}
For \ref{it:thm:9.4p1}, use Tarski-Vaught Test (\cref{lem:3.8}): if $\psi(z)$ is a consistent $L(M)$-formula, then it has parameters in $A_i$ for some $i$, so by \cref{rem:9.2}, $\M$ contains a witness. Condition \ref{it:thm:9.4p2} is satisfied by construction: none of the $A_i$ isolates $p(\bar{x})$, so $\M$ does not isolate $p(\bar x)$, so since $\M$ is a model, by \cref{rem:9.2} it does not realise $p(\bar{x})$.
\end{proof}
...
Let $\psi(z)$ be a consistent $L(A)$-formula. We build a sequence $\left< \psi_i(z) : i < \omega \right>$ starting with $\psi_0 \coloneqq \psi$ such that
\begin{enumerate}[label=(\roman*)]
...
We must show that it is possible to find such $\varphi(\bar{x})$. If no such $\varphi(\bar{x})$ exists, then for all $\varphi(\bar{x}) \in p(\bar x)$,
\[
\chi_i(\bar{x}, z) \land \psi_i(z) \to \varphi(\bar x).
\]
But this implies that 
\[
\exists z \left( \chi_i(\bar{x}, z) \land \chi_i(z) \right) \to p(\bar{x}).
\]
...
\item $\M$ is \named{atomic over $A$}\index{model!atomic} if for all $n \in \omega$ and $\bar a \in M^n$, the type $\hyperlink{def:tp}{\tp}(\bar a/A)$ is isolated. When $A = \emptyset$, then $\M$ is said to be \named{atomic}.
\end{enumerate}
\end{ndef}

\begin{nfact} \label{fact:9.6}
Let $\bar{a}, \bar{b} \in U^n$ and $A \subseteq U$. Suppose that $\hyperlink{def:tp}{\tp}(\bar{b}\bar{a}/A)$ is isolated, then $\tp(\bar b/A\bar{a})$ and $\tp(\bar{a}/A)$ are isolated.
\end{nfact}
...
\item $\exists \bar{x}\; \varphi(\bar{x},\bar{z})$ isolates $\tp(\bar{a}/A)$.
\end{enumerate}
\end{proof}
...
\end{nprop}
\begin{proof}
Let $\bar{b} \in M^m$. Then $\tp(\bar{b}\bar{a}/A)$ is isolated, so $\tp(\bar{b}/A\bar{a})$ is isolated by \cref{fact:9.6}.
\end{proof}
...
Suppose that $\M$ is not atomic over $A$, i.e.\@ that there is $\bar{b} \in M^n$ such that $\tp(\bar{b}/A)$ is not isolated. By the Omitting Type Theorem (\cref{thm:9.4}), there is a countable model $\N$ that omits $\tp(\bar{b}/A)$. But then $\M$ does not embed into $\N$, a contradiction to $\M$ being prime.
\end{proof}
