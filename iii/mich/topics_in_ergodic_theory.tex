...
\begin{thm}[Weyl]\hypertarget{thm:weyl}
  Let \marginnote{\emph{Lecture 9}}$P(x) = a_d x^d + \dotsb + a_1 x + a_0$ be a polynomial such that $a_j$ is irrational for at least one $j \neq 0$.
...
\begin{defi}[Conditional information relative to sub-$\sigma$-algebra]\hypertarget{def:i2}
  ...
  \begin{equation*}
    I_\mu(\eta \mid \mathcal{A}) = \sum_{A \in \eta} -\chi_A \cdot \log \mathbb{E}[\chi_A \mid \mathcal{A}].
  \end{equation*}
\end{defi}
\begin{eg}
  Suppose that $\mathcal{A}$ is generated by a \hyperlink{def:partition}{countable partition} $\xi \subset \mathcal{B}$. Then:
  \begin{align*}
    \hyperlink{def:i2}{I_\mu(\eta \mid \mathcal{A})} &= \sum_{A \in \eta} -\chi_A(x) \log \mathbb{E}[\chi_A \mid \mathcal{A}] \\
...
\end{proof}
%missing
\marginnote{\emph{Lecture 20}}
Missing content
% lecture 21
\marginnote{\emph{Lecture 21}}
