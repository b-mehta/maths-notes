...
\begin{defi}[$ij$-compression]\index{Compression}\hypertarget{def:comp}
...
\begin{eg}
  ...
  \begin{equation*}
    C_{12}(\mathcal{A}) = \set{123,134,234,135,147}.
  \end{equation*}
...
\begin{remark}
  ...
  \end{equation*}
\end{remark}
\begin{defi}[Left-compressed]\index{left-compressed}\hypertarget{def:lcomp}
...
\begin{nprop}\label{prop:5}
  ...
  Then $\exists$ \hyperlink{def:lcomp}{left-compressed} $\mathcal{B} \subset X^{(r)}$ with $|\mathcal{B}| = |\mathcal{A}|$ and $|\partial \mathcal{B}| \leq |\partial \mathcal{A}|$
\end{nprop}
\begin{proof}
  Among all $\mathcal{B} \subset X^{(r)}$ with $|\mathcal{B}| = |\mathcal{A}|$ and $|\partial \mathcal{B}| \leq |\partial \mathcal{A}|$,
  choose one with $\sum_{A \in \mathcal{B}} \sum_{x \in A} x$ minimal.
  Then $\mathcal{B}$ left-compressed, as if $C_{ij}(\mathcal{B}) \neq \mathcal{B}$ we contradict minimality.
\end{proof}
\begin{remark}
  Or apply a $C_{ij}$, then another, and so on - this must terminate.
  In fact, can apply each $C_{ij}$ at most once, if you chose a sensible order.
\end{remark}

Certainly initial segments of colex are left-compressed.
The converse is false - e.g.\ $\mathcal{A} = \{123,124,125,126,127\}$.

`Colex prefers $23$ to $14$' inspires:
For $U,V \subset X$ with $|U| = |V|$ and $U \cap V = \emptyset$, the $UV$-compression $C_{UV}$ is defined by
For $A \subset X$,
\begin{equation*}
  \begin{cases}
...
Say $\mathcal{A}$ is $UV$-compressed if $C_{UV}(\mathcal{A}) = \mathcal{A}$.

\begin{eg}
  If $\mathcal{A} = \{123,134,235,145,146,157\}$ then
  \begin{equation*}C_{23,14}(\mathcal{A}) = \{123,134,235,145,236,157\}.\end{equation*}
\end{eg}
Note that $|C_{UV}(\mathcal{A})| = |\mathcal{A}|$.

Sadly, $C_{UV}$ need not decrease $\partial$ - e.g.\ $\mathcal{A} = \{146,467\}$ has $|\partial\mathcal{A}| = 5$, but $C_{23,14}(\mathcal{A}) = \{236,147\}$ has $|\partial C{23,14}(\mathcal{A})| = 6$.
$C_{23,14}$ moved some things a long way.
However:
\begin{nprop}\label{prop:6}
  Let $\mathcal{A} \subset X^{(r)}$ and $U,V \subseteq X$ with $|U| = |V|$ and $U \cap V = \emptyset$.
  Suppose $\forall x \in U$ $\exists y \in V$ such that $\mathcal{A}$ is $(U-x, V-y)$-compressed.
