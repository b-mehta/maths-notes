    ...
    \begin{tikzpicture}
      \begin{scope}
        ...
        \begin{scope}
          ...
          \draw [very thin, fill=bred!40] (-1.6,-0.8) -- (0.2,-0.8) arc (0:90:2mm) -- (0,0.6) arc (-90:-180:2mm) -- (-1.6,0.8) -- cycle;
...
\begin{proof}
  ...
  Given $A \subset \hyperlink{def:qn}{Q_n}$ for $n > 1$ and $1 \leq i \leq n$: \textbf{Claim} $|\hyperlink{def:ebound}{\partial} B_i(A)| \leq |\partial A|$.
...
\end{ncor}
\begin{proof}
  The set $A = \mathcal{P}([n-1])$ shows $i(Q_n)\leq1$.
  ...
  But this is clear because $C \subset \mathbb{P}([n-1])$.
\end{proof}
...
\begin{defi}[Grid]\hypertarget{def:grid}
  ...
  For $k=2$, this is the discrete cube $Q_n$.
\end{defi}
Do \cref{thm:harper1} and \cref{thm:2.8} have analogues in \hyperlink{def:grid}{$[k]^n$}?

What is the best vertex-boundary? Take $[k]^2$ as an example:

This suggests that sets of the form
\begin{equation*}
  \set{x | \lvert x \rvert \leq r}
\end{equation*}
are best.
...
\end{eg}
\begin{aim}
  Initial segments of \hyperlink{def:simplic}{simplicial} minimise neighbourhood.
  In particular,
  \begin{equation*}
    |A| = |\set{x | |x|\leq r}| \implies |N(A)| \geq |\set{x | |x| \leq r+1}|.
  \end{equation*}
\end{aim}
For $A \subset [k]^n$ and $1 \leq i \leq n$, the $i$-sections of $A$ are the sets $A_1, \dotsc, A_k$ (or $A_1^{(i)}, \dotsc, A_k^{(i)}$) in $[k]^{n-1}$ given by
\begin{equation*}
  A_t = \set{(x_1, \dotsc, x_{n-1}) \in [k]^{n-1} | (x_1, x_2, \dotsc, x_{i-1}, t, x_i, x_{i+1}, \dotsc, x_n) \in A} \quad \forall 1 \leq t \leq k
\end{equation*}
% picture
\begin{defi}
  The $i$-compression $C_i(A) \subseteq [k]^n$ is defined by giving its $i$-sections:
  \begin{equation*}
    C_i(A)_t = \text{first |$A_t|$ points in simplicial order on $[k]^{n-1}$}.
  \end{equation*}
\end{defi}
Certainly $|C_i(A)| \leq |A|$.

Say $A$ is $i$-compressed if $C_i(A) = A$.
...
\begin{proof}
  ...
  For $1 \leq t \leq k$,
  \begin{equation*}
    N(A)_t = N(A_t) \cup A_{t-1} \cup A_{t+1} % from level t, from below, from above
  \end{equation*}
