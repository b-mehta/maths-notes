...
\end{defi}
\hypertarget{def:superr}For instance, we write $X^{(r)} = \set{A \subset X | |A| = r}$, so $X^{(r)}$ is a \hyperlink{def:ss}{set system} on $X$.
...
\begin{defi}[Neighbourhood]\hypertarget{def:neighbourhood}
  ...
  \begin{equation*}
    N(A) = A \cup \hyperlink{def:boundary}{b(A)} = \set{x \in G | d(x,A) \leq 1}
  \end{equation*}
...
\begin{defi}[Simplicial ordering]\hypertarget{def:simplic}
  We define the \named{simplicial order} on $\hyperlink{def:grid}{[k]^n}$ by setting $x<y$ if either $|x| < |y|$ or $|x| = |y|$ and $x_i > y_i$, where $i = \min\set{j | x_j \neq y_j}$.
