...
\begin{ex}
    ...
    \end{equation*}
    Let $\nu: A \in \mathscr{A} \mapsto \int_A f \, d\mu$ (for $f$ a \hyperlink{def:measFunc}{measurable} $X \to [0, +\infty]$).
...
\begin{proof}
    ...
    \begin{enumerate}[label=(\arabic*)]
        \item Let $\mathcal{C} = \{\text{open sets of the form } \prod_{i=1}^n (a_i, b_i), a_i, b_i \in \Q\}$, a countable set.
            ...
            Claim: Any open set $\mathcal{O} \subset \R^n$ can be covered with a countable union of elements of $\mathcal{C}$ with disjoint interiors.

            Use an inductive procedure: Split into $\Z^n$. 1. Keep cubes that are fully inside, discard ones that are fully outside. For borderline cubes, divide into $2^n$ cubes evenly and start 1 again.
            Check that $\mathcal{O} = \bigcup_{n, k} C_{n, k}$, and note that the hypercubes have dyadic, hence rational coordinates.
