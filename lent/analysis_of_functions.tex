...
\end{remark}
\begin{ex}
    Check linearity of the \hyperlink{def:integral}{integral}. Show Chebyshev's inequality:
    \begin{equation*}
...
\begin{proof}[Proof (1)]
    ...
    \begin{align*}
        ...
        \shortintertext{and} \\
        a>0, E_a &\coloneqq \set{x \in \R^n | Mf(x) > a}.
    \end{align*}
    Claim: $E_a$ is an open set, hence measurable.
    Take $x \in E_a$. $Mf(x) > a$, $\exists r >0$ such that $mf(x) > a$. So, $\frac{a}{mf(x)} < 1$.
    Pick $\epsilon > 0$ small such that $(\frac{r}{r+\epsilon}) > \frac{a}{mf(x, r)}$. Consider $y \in B(x, \epsilon)$, so $B(x, r) \subset B(y, r + \epsilon)$ by triangle inequality.
    \begin{align*}
        mf(y, r+\epsilon) &= \frac{1}{\mu(B(y, r+\epsilon))} \int_{B(y, r+\epsilon)} \abs{f} \, d\mu \\
                          &\geq (\frac{r}{r+\epsilon})^n \frac{1}{\mu(B(x,r))} \int_{B(x, r)} \abs{f} \, d\mu \geq (\frac{r}{r+\epsilon})^n mf(x, r) > a
    \end{align*}
    ...
    \textbf{Step 3}. $\mu(E_a) \leq \frac{3^n}{a} \norm{f}_{L^1(\R^n)}$.
    Sketch: Take $K$ compact, $K \subset E_a$ (inner regularity).
    For each $x \in K \quad \exists r_x, mf(x, r_x) > a$, $K \subset \bigcup_{x \in K} B(x, r_x) \subset \bigcup_{i=1}^N B(x_i, r_i) \subset \bigcup_{i \in J} B(x_i, 3 r_i)$.
    By Vitali covering lemma, $\mu(K) \leq \sum_{J} \mu(B(x_i, 3r_i)) \leq \frac{3^n}{a} \int \abs{f}$.
\end{proof}
\end{document}
