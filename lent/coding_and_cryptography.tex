...
\begin{proof}[Sketch proof]
    ...
    \begin{equation*}
        P\left(\abs{- \frac{1}{n} p(X_1, \dotsc, X_n) - H} < \epsilon\right) \geq P(T_n) \to 1 \quad \text{as } n \to \infty. \qedhere
    \end{equation*}
\end{proof}
...
\begin{proof}Let $\epsilon > 0$ and let $T_n \subset \Sigma^n$ be \hyperlink{def:typicalSet}{typical sets}.
    ...
    So,
    \begin{gather*}
        P(\left(X_1, \dotsc, X_n\right) \in T_n) \geq 2^{-n(H+\epsilon)} \abs{T_n} \\
    ...
    \begin{gather*}
        ...
        \begin{aligned}
            \implies P((x_1, \dotsc, x_n) \in A_n \cap T_n) &\leq 2^{-n (H-\epsilon)} \abs{A_n} \\
            \implies \frac{\log P(A_n \cap T_n)}{n} &\leq -(H-\epsilon) + \frac{\log \abs{A_n}}{n} \\
...
A code with size $m=1$ has \hyperlink{def:infoRate}{information rate} 0.
We aim to design code with both a large information rate and a small error rate, which are contradicting aims.
The error rate depends on the decoding rule. We consider 3 possible rules.
\begin{enumerate}[label=(\roman*)]
    \item The \hypertarget{def:idealObserver}{\textbf{ideal observer}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ maximising $P(c\text{ sent} \mid x\text{ received})$.
    \item The \hypertarget{def:maximumLikelihood}{\textbf{maximum likelihood}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ maximising $P(x\text{ received} \mid c\text{ sent})$.
    \item The \hypertarget{def:minimumDistance}{\textbf{minimum distance}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ minimising $\#\set{1 \leq i \leq n | x_i \neq c_i}$.
\end{enumerate}
\begin{remark}
...
\begin{proof}
    By Bayes' rule,
    \begin{align*}
        P(c \text{ sent} \mid x \text{ received}) &= \frac{P(c \text{ sent}, x \text{ received})}{P(x \text{ received})} \\
                                               &= \frac{P(c \text{ sent})}{P(x \text{ received})} P(x \text{ received} \mid c \text{ sent}).
    \end{align*}
    We suppose $P(c \text{ sent})$ is independent of $c$. So for fixed $x$, maximising $P(c \text{ sent} \mid x \text{ received}) $ is the same as maximising $P(x \text{ received} \mid c \text{ sent})$.
\end{proof}

\begin{defi}
    Let $x, y \in \{0, 1\}^n$. Then \textbf{Hamming distance} between $x$ and $y$ is $d(x, y) = \# \set{1 \leq i \leq n | x_i \neq y_i}$.
\end{defi}
\begin{nlemma}\label{lem:2.2}
    If $p < \frac{1}{2}$, then \hyperlink{def:maximumLikelihood}{maximum likelihood} and \hyperlink{def:minimumDistance}{minimum distance} agree.
\end{nlemma}
\begin{proof}
    Suppose $d(x, c) = r$.
    \begin{equation*}
        P(x \text{ received} \mid c \text{ sent}) = p^r (1-p)^{n-r} = (1-p)^n \left(\frac{p}{1-p}\right)^r
    \end{equation*}
    Since $p < \frac{1}{2}$, $\frac{p}{1-p} < 1$. So choosing $c$ to maximise $P(x \text{ received} \mid c \text{ sent})$ is the same as choosing $c$ to minimise $d(x, c)$.
\end{proof}
...
\begin{eg}
    ...
    Clearly \hyperlink{def:minimumDistance}{minimum distance} and therefore \hyperlink{def:maximumLikelihood}{maximum likelihood} (by \cref{lem:2.2}) say decode as $111$.
    For \hyperlink{def:idealObserver}{ideal observer:}
    \begin{align*}
        P(000 \text{ sent} \mid 110 \text{ received}) &= \frac{P(000 \text{ sent} , 110 \text{ received})}{P(110 \text{ received})} \\
                                                   &= \frac{\alpha p^2 (1-p)}{\alpha p^2 (1-p) + (1-\alpha) p (1-p)^2} \\
                                                   &= \frac{\alpha p}{\alpha p + (1-\alpha) (1-p)} \\
                                                   &= \frac{9/40}{9/40 + 3/40} = \frac{3}{4} \\
        P(000 \text{ sent} \mid 110 \text{ received}) &= \frac{(1-\alpha) p^2 (1-p)}{(1-\alpha) p^2 (1-p) + \alpha p (1-p)^2} \\
...
\end{remark}
From now on, we use \hyperlink{def:minimumDistance}{minimum distance} decoding.
\begin{defi}
...
\begin{eg} \leavevmode
    \begin{enumerate}[label=\arabic*.]
        \item The repetition code of length $n$ has:
            \begin{equation*}
                C = \{\underbrace{0\dotsm0}_n, \underbrace{1 \dotsm1}_n\}
            \end{equation*}
            This is an \hyperlink{def:binaryCode}{$[n, 2]$-code}. It is \hyperlink{def:errorDet}{$n-1$ error detecting}, and $\floor{\frac{n-1}{2}}$ \hyperlink{def:errorCor}{error correcting}.
            But it has \hyperlink{def:infoRate}{information rate} $= \frac{1}{n}$.
        \item The simple parity check code of length $n$ (also known as paper tape code).
            We view $\{0, 1\} = \mathbb{F}_2 = \Z/2\Z$ (i.e.\ do arithmetic modulo $2$).
            \begin{equation*}
                C = \Set{(x_1, \dotsc, x_n) \in \{0, 1\}^n | \sum_{i=1}^n x_i = 0}.
            \end{equation*}
            This is a $[n, 2^{n-1}]$ code. It is $1$-\hyperlink{def:errorDet}{error detecting} and $0$-\hyperlink{def:errorCor}{error correcting}.
            It has information rate $\frac{n-1}{n}$.
    \end{enumerate}
\end{eg}
\begin{remark}
    Suppose we change our code $C \subset \{0, 1\}^n$ by using the same permutation to reorder each codeword.
