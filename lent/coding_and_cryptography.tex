...
\begin{enumerate}[label=(\roman*)]
    \item The \hypertarget{def:idealObserver}{\textbf{ideal observer}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ maximising $\Prob(c\text{ sent} \mid x\text{ received})$.
    \item The \hypertarget{def:maximumLikelihood}{\textbf{maximum likelihood}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ maximising $\Prob(x\text{ received} \mid c\text{ sent})$.
    \item The \hypertarget{def:minimumDistanceRule}{\textbf{minimum distance}} decoding rule decodes $x \in \{0, 1\}^n$ as the codeword $c$ minimising $\#\set{1 \leq i \leq n | x_i \neq c_i}$.
\end{enumerate}
\begin{remark}
...
\begin{eg} \leavevmode
    \begin{enumerate}[label=\arabic*.]
        ...
        \item \hypertarget{def:hammingCode}{Hamming's original} code (1950). % this example was lecture 7
            ...
            It has \hyperlink{def:infoRate}{information rate} $\frac{\log m} {n}$.
            ...
            \begin{align*}
                ...
                z_4 = x_4 + x_5 + x_6 + x_7.
            \end{align*}
            If $x \in C$, then $z = (0,0,0)$. If $d(x, c) = 1$ for some $c \in C$, then the place where $x$ and $c$ differ is given by $z_1 + 2 z_2 + 4z_4$ (not modulo 2).
            Since if $x = c+ e_i$ where $e_i = 0\dotsm010\dotsm 0$ ($1$ in the $i$th place), then the syndrome of $x$ is the syndrome of $e_i$.
...
Recall $d(x, y) = \#\set{1 \leq i \leq n | x_i \neq y_i}$.
...
\begin{defi}[Minimum distance]\label{def:minimumDistanceCode}
    The \textbf{minimum distance} of a code $C$ is the smallest Hamming distance between distinct codewords.
\end{defi}
\begin{notation}
    An \hyperlink{def:binaryCode}{$[n, m]$-code} with \hyperlink{def:minimumDistanceCode}{minimum distance} $d$ is sometimes called an \hyperlink{def:nmdCode}{$[n, m, d]$-code}.
\end{notation}
...
\begin{proof}\leavevmode
    \begin{enumerate}[label=(\roman*)]
        \item If $x \in \mathbb{F}_2^n$ and $c \in C$ with $1 \leq d(x, c) \leq d-1$ then $x \notin C$. So errors are detected.
            Suppose $c_1, c_2 \in C$ with $d(c_1, c_2) = d$. Then $c_1$ can be corrupted to $c_2$ with just $d$ errors, this set of errors will not be detected.
...
\begin{eg}
    ...
    \begin{enumerate}[label=\arabic*)]
        ...
        \item \hyperlink{def:hammingCode}{Hamming's original} $[7, 16]$-code is $1$-error correcting $\implies d \geq 3$. Since 0000000 and 1110000 are both valid codewords, $d=3$. That is, it is a $[7, 16, 3]$-code.
    \end{enumerate}
\end{eg}
...
\begin{enumerate}[label=\roman*)]
    ...
    \item Fix $i \leq i \leq n$ and $a \in \{0, 1\}$. The shortened code is
        \begin{equation*}
